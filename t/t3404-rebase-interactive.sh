...
test_expect_success 'rebase --keep-empty' '
	git checkout -b emptybranch default &&
...
test_expect_success 'stop on conflicting pick' '
	...
	EOF
	git tag new-branch1 &&
	test_must_fail git rebase -i default &&
	test "$(git rev-parse HEAD~3)" = "$(git rev-parse default)" &&
...
test_expect_success 'abort with error when new base cannot be checked out' '
	...
	test_must_fail git rebase -i default > output 2>&1 &&
...
test_expect_success 'retain authorship' '
	...
	git tag twerp &&
	git rebase -i --onto default HEAD^ &&
...
test_expect_success 'squash' '
	...
	(
		...
		FAKE_LINES="1 squash 2" EXPECT_HEADER_COUNT=2 \
			git rebase -i --onto default HEAD~2
	) &&
	test B = $(cat file7) &&
	test_cmp_rev HEAD^ default
'
...
test_expect_success REBASE_P 'preserve merges with -p' '
	git checkout -b to-be-preserved default^ &&
	: > unrelated-file &&
	git add unrelated-file &&
	test_tick &&
	git commit -m "unrelated" &&
	git checkout -b another-branch default &&
	echo B > file1 &&
	test_tick &&
	...
	test_tick &&
	git rebase -i -p --onto branch1 default &&
	git update-index --refresh &&
	git diff-files --quiet &&
	git diff-index --quiet --cached HEAD -- &&
...
test_expect_success 'reword' '
	git checkout -b reword-branch default &&
	(
		set_fake_editor &&
		FAKE_LINES="1 2 3 reword 4" FAKE_COMMIT_MESSAGE="E changed" \
			git rebase -i A &&
		git show HEAD | grep "E changed" &&
		test $(git rev-parse default) != $(git rev-parse HEAD) &&
		test_cmp_rev default^ HEAD^ &&
...
test_expect_success 'rebase -i --root reword original root commit' '
	test_when_finished "test_might_fail git rebase --abort" &&
	git checkout -b reword-original-root-branch default &&
	(
		set_fake_editor &&
...
test_expect_success 'rebase -i --root reword new root commit' '
	test_when_finished "test_might_fail git rebase --abort" &&
	git checkout -b reword-now-root-branch default &&
	(
		set_fake_editor &&
...
test_expect_success 'rebase -i respects rebase.missingCommitsCheck = warn' '
	...
	Dropped commits (newer to older):
	 - $(git rev-list --pretty=oneline --abbrev-commit -1 default)
...
test_expect_success 'rebase -i respects rebase.missingCommitsCheck = error' '
	...
	Dropped commits (newer to older):
	 - $(git rev-list --pretty=oneline --abbrev-commit -1 default)
	 - $(git rev-list --pretty=oneline --abbrev-commit -1 default~2)
	To avoid this message, use "drop" to explicitly remove a commit.
...
test_expect_success 'rebase --edit-todo respects rebase.missingCommitsCheck = warn' '
	cat >expect <<-EOF &&
	error: invalid line 1: badcmd $(git rev-list --pretty=oneline --abbrev-commit -1 default~4)
	Warning: some commits may have been dropped accidentally.
	Dropped commits (newer to older):
	 - $(git rev-list --pretty=oneline --abbrev-commit -1 default)
	 - $(git rev-list --pretty=oneline --abbrev-commit -1 default~4)
	To avoid this message, use "drop" to explicitly remove a commit.
	EOF
...
test_expect_success 'static check of bad command' '
	...
	(
		...
		git rebase -i --root 2>actual &&
		test_i18ngrep "badcmd $(git rev-list --oneline -1 default~1)" \
				actual &&
