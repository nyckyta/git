...
test_expect_success 'simple A..B A..C (unmodified)' '
	git range-diff --no-color default..topic default..unmodified \
		>actual &&
	cat >expect <<-EOF &&
...
test_expect_success 'simple A B C (unmodified)' '
	git range-diff --no-color default topic unmodified >actual &&
	# same "expect" as above
	test_cmp expect actual
'

test_expect_success 'trivial reordering' '
	git range-diff --no-color default topic reordered >actual &&
	cat >expect <<-EOF &&
...
test_expect_success 'removed a commit' '
	git range-diff --no-color default topic removed >actual &&
	cat >expect <<-EOF &&
...
test_expect_success 'added a commit' '
	git range-diff --no-color default topic added >actual &&
	cat >expect <<-EOF &&
...
test_expect_success 'new base, A B C' '
	git range-diff --no-color default topic rebased >actual &&
	cat >expect <<-EOF &&
...
test_expect_success 'new base, B...C' '
	# this syntax includes the commits from default!
...
test_expect_success 'no commits on one side' '
	...
	git range-diff default HEAD@{1} HEAD
'
...
for prev in topic default..topic
do
	test_expect_success "format-patch --range-diff=$prev" '
		git format-patch --cover-letter --range-diff=$prev \
			default..unmodified >actual &&
...
test_expect_success 'basic with modified format.pretty with suffix' '
	git -c format.pretty="format:commit %H%d%n" range-diff \
		default..topic default..unmodified
'
...
test_expect_success 'range-diff with --no-notes' '
	...
	git notes add -m "unmodified note" unmodified &&
	test_when_finished git notes remove topic unmodified &&
	git range-diff --no-color --no-notes default..topic default..unmodified \
		>actual &&
	cat >expect <<-EOF &&
	1:  $(test_oid t1) = 1:  $(test_oid u1) s/5/A/
	2:  $(test_oid t2) = 2:  $(test_oid u2) s/4/A/
...
test_expect_success 'range-diff with multiple --notes' '
	...
	git range-diff --no-color --notes=note1 --notes=note2 default..topic default..unmodified \
		>actual &&
	sed s/Z/\ /g >expect <<-EOF &&
	1:  $(test_oid t1) = 1:  $(test_oid u1) s/5/A/
	2:  $(test_oid t2) = 2:  $(test_oid u2) s/4/A/
