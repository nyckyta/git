...
test_expect_success 'stash branch' '
	...
	test $(git rev-parse HEAD) = $(git rev-parse default^) &&
	...
	git diff default..stashbranch >output &&
...
test_expect_success 'stash branch - no stashes on stack, stash-like argument' '
	...
	test_when_finished "git reset --hard HEAD && git checkout default &&
...
test_expect_success 'branch: do not drop the stash if the branch exists' '
	...
	echo bar >file &&
	git stash &&
	test_must_fail git stash branch default stash@{0} &&
...
test_expect_success 'push -m shows right message' '
	...
	echo "stash@{0}: On default: test message" >expect &&
...
test_expect_success 'push -m also works without space' '
	...
	echo "stash@{0}: On default: unspaced test message" >expect &&
	git stash list -1 >actual &&
	test_cmp expect actual
'
...
test_expect_success 'push -mfoo uses right message' '
	...
	echo "stash@{0}: On default: test mfoo" >expect &&
	git stash list -1 >actual &&
	test_cmp expect actual
'
...
test_expect_success 'push --message foo is synonym for -mfoo' '
	...
	echo "stash@{0}: On default: test message foo" >expect &&
	git stash list -1 >actual &&
	test_cmp expect actual
'
...
test_expect_success 'push -m shows right message' '
	...
	echo "stash@{0}: On default: test m foo" >expect &&
	git stash list -1 >actual &&
	test_cmp expect actual
'
...
test_expect_success 'create stores correct message' '
	...
	echo "On default: create test message" >expect &&
...
test_expect_success 'create with multiple arguments for the message' '
	...
	echo "On default: test untracked" >expect &&
	git show --pretty=%s -s ${STASH_ID} >actual &&
	test_cmp expect actual
'
