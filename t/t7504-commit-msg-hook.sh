...
test_expect_success 'merge fails with failing hook' '
	...
	test_must_fail git merge --allow-unrelated-histories default &&
...
test_expect_success 'merge bypasses failing hook with --no-verify' '
	...
	git add file2 &&
	git commit --no-verify file2 -m in-side-branch &&
	git merge --no-verify --allow-unrelated-histories default &&
	commit_msg_is "Merge branch '\''default'\'' into newbranch"
'
...
test_expect_success 'hook called in git-merge picks up commit message' '
	...
	git add file2 &&
	git commit --no-verify file2 -m in-side-branch &&
	git merge --allow-unrelated-histories default &&
	commit_msg_is "new message"
'
...
test_expect_failure 'merge --continue remembers --no-verify' '
	...
	echo a >file2 &&
	git add file2 &&
	git commit --no-verify -m "add file2 to default" &&
	git checkout -b newbranch default^ &&
	echo b >file2 &&
	git add file2 &&
	git commit --no-verify file2 -m in-side-branch &&
	git merge --no-verify -m not-rewritten-by-hook default &&
	# resolve conflict:
	echo c >file2 &&
	git add file2 &&
	git merge --continue &&
