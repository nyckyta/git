...
test_expect_success '"add" an existing worktree' '
	...
	test_must_fail git worktree add --detach existing default
'

test_expect_success '"add" an existing empty worktree' '
	mkdir existing_empty &&
	git worktree add --detach existing_empty default
'
...
test_expect_success '"add" refuses to checkout locked branch' '
	test_must_fail git worktree add zere default &&
	! test -d zere &&
...
test_expect_success 'checking out paths not complaining about linked checkouts' '
	...
	cd existing_empty &&
	echo dirty >>init.t &&
	git checkout default -- init.t
	)
'

test_expect_success '"add" worktree' '
	git rev-parse HEAD >expect &&
	git worktree add --detach here default &&
	(
		cd here &&
...
test_expect_success '"add" worktree with lock' '
	git rev-parse HEAD >expect &&
	git worktree add --detach --lock here-with-lock default &&
...
test_expect_success '"add" from a linked checkout' '
	(
		cd here &&
		git worktree add --detach nested-here default &&
		cd nested-here &&
		git fsck
	)
'
...
test_expect_success '"add" worktree creating new branch' '
	git worktree add -b newmaster there default &&
	(
		cd there &&
		test_cmp ../init.t init.t &&
...
test_expect_success '"add" from a bare repo' '
	(
		git clone --bare . bare &&
		cd bare &&
		git worktree add -b bare-default ../there2 default
	)
'
...
test_expect_success 'checkout from a bare repo without "add"' '
	(
		cd bare &&
		test_must_fail git checkout default
	)
'
...
test_expect_success '"add" default branch of a bare repo' '
	(
		...
		cd bare2 &&
		git worktree add ../there3 default
	)
'
...
test_expect_success 'checkout with grafts' '
	...
	git worktree add --detach grafted default &&
...
test_expect_success '"add" -b/-B mutually exclusive' '
	test_must_fail git worktree add -b poodle -B poodle bamboo default
'

test_expect_success '"add" -b/--detach mutually exclusive' '
	test_must_fail git worktree add -b poodle --detach bamboo default
'
...
test_expect_success '"add -B" fails if the branch is checked out' '
	...
	test_must_fail git worktree add -B newmaster bamboo default &&
	git rev-parse newmaster >after &&
	test_cmp before after
'

test_expect_success 'add -B' '
	git worktree add -B poodle bamboo2 default^ &&
	git -C bamboo2 symbolic-ref HEAD >actual &&
	echo refs/heads/poodle >expected &&
	test_cmp expected actual &&
	test_cmp_rev default^ poodle
'

test_expect_success 'add --quiet' '
	git worktree add --quiet another-worktree default 2>actual &&
...
test_expect_success '--track sets up tracking' '
	...
	git worktree add --track -b track track default &&
	test_branch_upstream track . default
'

# setup remote repository $1 and repository $2 with $1 set up as
# remote.  The remote has two branches, default and foo.
setup_remote_repo () {
	git init $1 &&
	(
		cd $1 &&
