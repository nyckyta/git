...
test_expect_success 'verify that .promisor file contains refs fetched' '
	...
	grep "$(cat headhash) refs/heads/default" $(cat promisorlist)
'

# checkout default to force dynamic object fetch of blobs at HEAD.
test_expect_success 'verify checkout with dynamic object fetch' '
	git -C pc1 rev-list --quiet --objects --missing=print HEAD >observed &&
	test_line_count = 4 observed &&
	git -C pc1 checkout default &&
	git -C pc1 rev-list --quiet --objects --missing=print HEAD >observed &&
...
test_expect_success 'push new commits to server' '
	...
	git -C src blame default -- file.1.txt >expect.blame &&
	git -C src push -u srv default
'
...
test_expect_success 'partial fetch inherits filter settings' '
	...
	git -C pc1 rev-list --quiet --objects --missing=print \
		default..origin/default >observed &&
	test_line_count = 5 observed
'

# force dynamic object fetch using diff.
# we should only get 1 new blob (for the file in origin/default).
test_expect_success 'verify diff causes dynamic object fetch' '
	git -C pc1 diff default..origin/default -- file.1.txt &&
	git -C pc1 rev-list --quiet --objects --missing=print \
		 default..origin/default >observed &&
	test_line_count = 4 observed
'
...
test_expect_success 'verify blame causes dynamic object fetch' '
	git -C pc1 blame origin/default -- file.1.txt >observed.blame &&
...
test_expect_success 'use fsck before and after manually fetching a missing subtree' '
	...
	git -C dst rev-list --missing=allow-any --objects default \
		>fetched_objects &&
	...
	git -C dst rev-list --missing=error --objects default >fetched_objects &&
...
test_expect_success 'partial clone with sparse filter succeeds' '
	...
	git clone --no-local --bare \
		  --filter=sparse:oid=default:only-one \
		  sparse-src dst.git &&
	(
		cd dst.git &&
...
test_expect_success 'partial clone with unresolvable sparse filter fails cleanly' '
	...
	test_must_fail git clone --no-local --bare \
				 --filter=sparse:oid=default:no-such-name \
				 sparse-src dst.git 2>err &&
	test_i18ngrep "unable to access sparse blob in .default:no-such-name" err &&
	test_must_fail git clone --no-local --bare \
				 --filter=sparse:oid=default \
				 sparse-src dst.git 2>err &&
