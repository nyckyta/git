...
cat >expect <<'EOF'
Your branch is behind 'default' by 1 commit, and can be fast-forwarded.
  (use "git pull" to update your local branch)
EOF
test_expect_success 'tracking count is accurate after orphan check' '
	reset &&
	git branch child default^ &&
	git config branch.child.remote . &&
	git config branch.child.merge refs/heads/default &&
	git checkout child^ &&
...
test_expect_success 'describe_detached_head prints no SHA-1 ellipsis when not asked to' "
	...
	commit=$(git rev-parse --short=12 default^) &&
	commit2=$(git rev-parse --short=12 default~2) &&
	commit3=$(git rev-parse --short=12 default~3) &&
