...
test_expect_success 'one push option works for a single branch' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		git push --push-option=asdf up default
	) &&
	test_refs default default &&
	echo "asdf" >expect &&
...
test_expect_success 'push option denied by remote' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		test_must_fail git push --push-option=asdf up default
	) &&
	test_refs default HEAD@{1}
'
...
test_expect_success 'two push options work' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		git push --push-option=asdf --push-option="more structured text" up default
	) &&
	test_refs default default &&
...
test_expect_success 'push options and submodules' '
	...
	git -C parent push \
		...
		--recurse-submodules=on-demand up default &&

	git -C upstream rev-parse --verify default >expect &&
	git -C parent/workbench rev-parse --verify default >actual &&
	test_cmp expect actual &&

	git -C parent_upstream rev-parse --verify default >expect &&
	git -C parent rev-parse --verify default >actual &&
	test_cmp expect actual &&
...
test_expect_success 'default push option' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		git -c push.pushOption=default push up default
	) &&
	test_refs default default &&
	echo "default" >expect &&
	test_cmp expect upstream/.git/hooks/pre-receive.push_options &&
...
test_expect_success 'two default push options' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		git -c push.pushOption=default1 -c push.pushOption=default2 push up default
	) &&
	test_refs default default &&
	printf "default1\ndefault2\n" >expect &&
	test_cmp expect upstream/.git/hooks/pre-receive.push_options &&
...
test_expect_success 'push option from command line overrides from-config push option' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		git -c push.pushOption=default push --push-option=manual up default
	) &&
	test_refs default default &&
	echo "manual" >expect &&
	test_cmp expect upstream/.git/hooks/pre-receive.push_options &&
...
test_expect_success 'empty value of push.pushOption in config clears the list' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		git -c push.pushOption=default1 -c push.pushOption= -c push.pushOption=default2 push up default
	) &&
	test_refs default default &&
...
test_expect_success 'invalid push option in config' '
	...
	(
		...
		git push --mirror up &&
		test_commit two &&
		test_must_fail git -c push.pushOption push up default
	) &&
	test_refs default HEAD@{1}
'
...
test_expect_success 'push options keep quoted characters intact (direct)' '
	...
	git -C workbench push --push-option="\"embedded quotes\"" up default &&
...
test_expect_success 'push option denied properly by http server' '
	...
	test_must_fail git -C test_http_clone push --push-option=asdf origin default 2>actual &&
	test_i18ngrep "the receiving end does not support push options" actual &&
	git -C test_http_clone push origin default
'
...
test_expect_success 'push options work properly across http' '
	...
	test_commit -C test_http_clone one &&
	git -C test_http_clone push origin default &&
	git -C "$HTTPD_DOCUMENT_ROOT_PATH"/upstream.git rev-parse --verify default >expect &&
	git -C test_http_clone rev-parse --verify default >actual &&
	test_cmp expect actual &&

	test_commit -C test_http_clone two &&
	git -C test_http_clone push --push-option=asdf --push-option="more structured text" origin default &&
	printf "asdf\nmore structured text\n" >expect &&
...
test_expect_success 'push options keep quoted characters intact (http)' '
	...
	test_commit -C test_http_clone one &&
	git -C test_http_clone push --push-option="\"embedded quotes\"" origin default &&
	echo "\"embedded quotes\"" >expect &&
	test_cmp expect "$HTTPD_DOCUMENT_ROOT_PATH"/upstream.git/hooks/pre-receive.push_options
'
