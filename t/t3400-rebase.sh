...
test_expect_success 'prepare repository with topic branches' '
	...
	git mv A D/A &&
	git commit -m "Move A." &&
	git checkout -b my-topic-branch default &&
	echo Second >B &&
...
test_expect_success 'rebase against default' '
	git reset --hard HEAD &&
	git rebase default
'
...
test_expect_success 'rebase off of the previous branch using "-"' '
	...
	git merge-base default HEAD >expect.forkpoint &&

	git checkout default &&
	git checkout HEAD^ &&
	git rebase - >actual.messages &&
	git merge-base default HEAD >actual.forkpoint &&
...
test_expect_success 'rebase a single mode change' '
	...
	GIT_TRACE=1 git rebase default
'
...
test_expect_success 'default to common base in @{upstream}s reflog if no upstream arg (--merge)' '
	git checkout -b def-base default &&
...
test_expect_success 'rebase --apply -q is quiet' '
	...
	git rebase --apply -q default >output.out 2>&1 &&
	test_must_be_empty output.out
'

test_expect_success 'rebase --merge -q is quiet' '
	git checkout -B quiet topic &&
	git rebase --merge -q default >output.out 2>&1 &&
	test_must_be_empty output.out
'
...
test_expect_success 'rebase with "From " line in commit message' '
	git checkout -b preserve-from default~1 &&
	cat >From_.msg <<EOF &&
...
test_expect_success 'switch to branch checked out here' '
	git checkout default &&
	git rebase default default
'

test_expect_success 'switch to branch not checked out' '
	git checkout default &&
	git branch other &&
	git rebase default other
'

test_expect_success 'refuse to switch to branch checked out elsewhere' '
	git checkout default &&
	git worktree add wt &&
	test_must_fail git -C wt rebase default default 2>err &&
