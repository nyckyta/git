...
test_expect_success 'more setup' '
	...
	git add subdir/new &&
	test_tick &&
	git commit -m "subdir on default" subdir/new &&
	git rm A.t &&
	test_tick &&
	git commit -m "again subdir on default" &&
	git merge branch
'
...
test_expect_success 'author information is preserved' '
	...
	(sane_unset GIT_AUTHOR_NAME &&
	 git filter-branch -f --msg-filter "cat; \
			test \$GIT_COMMIT != $(git rev-parse default) || \
			echo Hallo" \
		preserved-author) &&
...
test_expect_success "remove a certain author's commits" '
	...
	cnt1=$(git rev-list default | wc -l) &&
...
test_expect_success 'barf on invalid name' '
	test_must_fail git filter-branch -f default xy-problem &&
...
test_expect_success '"map" works in commit filter' '
	git filter-branch -f --commit-filter "\
		...
		actual=\$(echo \"\$@\" | sed \"s/^.*-p //\") &&
		test \$mapped = \$actual &&
		git commit-tree \"\$@\";" default~2..default &&
...
test_expect_success 'setup --prune-empty comparisons' '
	git checkout --orphan default-no-a &&
	git rm -rf . &&
	unset test_tick &&
	test_tick &&
	GIT_COMMITTER_DATE="@0 +0000" GIT_AUTHOR_DATE="@0 +0000" &&
...
test_expect_success '--remap-to-ancestor with filename filters' '
	...
	git filter-branch -f --remap-to-ancestor \
		moved-foo moved-bar A..default \
		-- -- foo &&
	test $(git rev-parse moved-foo) = $(git rev-parse moved-bar) &&
	test $(git rev-parse moved-foo) = $(git rev-parse default^) &&
...
test_expect_success 'automatic remapping to ancestor with filename filters' '
	...
	git filter-branch -f \
		moved-foo2 moved-bar2 A..default \
		-- -- foo &&
	test $(git rev-parse moved-foo2) = $(git rev-parse moved-bar2) &&
	test $(git rev-parse moved-foo2) = $(git rev-parse default^) &&
