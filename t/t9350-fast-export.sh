...
test_expect_success 'fast-export | fast-import' '
	...
	(cd new &&
	 git fast-import &&
	 test $default = $(git rev-parse --verify refs/heads/default) &&
...
test_expect_success 'fast-export default~2..default' '

	git fast-export default~2..default >actual &&
	sed "s/default/partial/" actual |
		(cd new &&
		 git fast-import &&
		 test $default != $(git rev-parse --verify refs/heads/partial) &&
		 git diff --exit-code default partial &&
		 git diff --exit-code default^ partial^ &&
		 test_must_fail git rev-parse partial~2)

'

test_expect_success 'fast-export --reference-excluded-parents default~2..default' '

	git fast-export --reference-excluded-parents default~2..default >actual &&
	grep commit.refs/heads/default actual >commit-count &&
	test_line_count = 2 commit-count &&
	sed "s/default/rewrite/" actual |
		(cd new &&
		 git fast-import &&
		 test $default = $(git rev-parse --verify refs/heads/rewrite))
'

test_expect_success 'fast-export --show-original-ids' '

	git fast-export --show-original-ids default >output &&
	grep ^original-oid output| sed -e s/^original-oid.// | sort >actual &&
	git rev-list --objects default muss >objects-and-names &&
...
test_expect_success 'fast-export --show-original-ids | git fast-import' '
	...
	git fast-export --show-original-ids default muss | git fast-import --quiet &&
	test $default = $(git rev-parse --verify refs/heads/default) &&
...
test_expect_success 'submodule fast-export | fast-import' '
	...
	SUBENT1=$(git ls-tree default^ sub) &&
	SUBENT2=$(git ls-tree default sub) &&
	rm -rf new &&
	mkdir new &&
	git --git-dir=new/.git init &&
	...
	(cd new &&
	 git fast-import &&
	 test "$SUBENT1" = "$(git ls-tree refs/heads/default^ sub)" &&
	 test "$SUBENT2" = "$(git ls-tree refs/heads/default sub)" &&
...
test_expect_success 'fast-export | fast-import when default is tagged' '
...
test_expect_failure 'no exact-ref revisions included' '
	(
		cd limit-by-paths &&
		git fast-export default~2..default~1 > output &&
		test_cmp expected output
	)
'
...
test_expect_success 'handling tags of blobs' '
	git tag -a -m "Tag of a blob" blobtag $(git rev-parse default:file) &&
...
test_expect_success 'directory becomes symlink'        '
	...
	(
		cd dirtosymlink &&
		git fast-export default -- foo |
		(cd ../result && git fast-import --quiet)
	) &&
	(cd result && git show default:foo)
'
...
test_expect_success 'avoid uninteresting refs' '
	> tmp-marks &&
	git fast-export --import-marks=tmp-marks \
		--export-marks=tmp-marks default > /dev/null &&
	git tag v1.0 &&
	git branch uninteresting &&
	echo bump > file &&
	git commit -a -m bump &&
	git fast-export --import-marks=tmp-marks \
		--export-marks=tmp-marks ^uninteresting ^v1.0 default > actual &&
	test_cmp expected actual
'
...
test_expect_success 'refs are updated even if no commits need to be exported' '
	...
	git fast-export --import-marks=tmp-marks \
		--export-marks=tmp-marks default > /dev/null &&
	git fast-export --import-marks=tmp-marks \
		--export-marks=tmp-marks default > actual &&
	test_cmp expected actual
'

test_expect_success 'use refspec' '
	git fast-export --refspec refs/heads/default:refs/heads/foobar default >actual2 &&
...
test_expect_success 'merge commit gets exported with --import-marks' '
	...
	(
		...
		git checkout default &&
		test_commit on-default &&
		test_tick &&
		git merge --no-ff -m Yeah topic &&

		echo ":1 $(git rev-parse HEAD^^)" >marks &&
		git fast-export --import-marks=marks default >out &&
		grep Yeah out
	)
'
