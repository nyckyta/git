...
test_expect_success setup '
	...
	git format-patch --stdout default >lorem-move.patch &&
	git format-patch --no-prefix --stdout default >lorem-zero.patch &&
...
test_expect_success 'am changes committer and keeps author' '
	...
	git diff --exit-code default..HEAD &&
	git diff --exit-code default^..HEAD^ &&
	compare author default HEAD &&
	compare author default^ HEAD^ &&
...
test_expect_success 'am and .gitattibutes' '
	...
	(
		...
		git checkout test &&
		git format-patch --stdout default..HEAD >patches &&
