...
test_expect_success 'check split_cmdline return' "
	...
	git config branch.default.mergeoptions 'echo \"' &&
...
test_expect_success '--show-origin blob ref' '
	cat >expect <<-\EOF &&
	blob:default:custom.conf	user.custom=true
	EOF
	git add "$CUSTOM_CONFIG_FILE" &&
	git commit -m "new config file" &&
	git config --blob=default:"$CUSTOM_CONFIG_FILE" --show-origin --list >output &&
	test_cmp expect output
'
