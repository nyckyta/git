...
#  HEAD - two
#  refs/for/refs/heads/default - one
#  refs/heads/default - three
test_expect_success 'setup repo with odd suffix ref' '
	echo content >file &&
	git add . &&
	git commit -m one &&
	git update-ref refs/for/refs/heads/default HEAD &&
	echo content >>file &&
	git commit -a -m two &&
	echo content >>file &&
...
test_expect_success 'suffix ref is ignored during fetch' '
	...
	echo three >expect &&
	git --git-dir=suffix log -1 --format=%s refs/heads/default >actual &&
