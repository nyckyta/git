...
test_expect_success C_LOCALE_OUTPUT 'remote information for the origin' '
	(
		...
		check_remote_track origin default side &&
		check_tracking_branch origin HEAD default side
	)
'
...
test_expect_success 'add another remote' '
	(
		...
		check_tracking_branch second default side another &&
...
test_expect_success C_LOCALE_OUTPUT 'check remote-tracking' '
	(
		cd test &&
		check_remote_track origin default side &&
		check_remote_track second default side another
	)
'
...
test_expect_success 'remove remote' '
	(
		cd test &&
		git symbolic-ref refs/remotes/second/HEAD refs/remotes/second/default &&
...
test_expect_success 'remove remote with a branch without configured merge' '
	...
	(
		...
		git fetch two &&
		git checkout -b second two/default^0 &&
...
* remote origin
  ...
  HEAD branch: default
  Remote branches:
    default new (next fetch will store in remotes/origin)
    side    tracked
  Local branches configured for 'git pull':
    ahead    merges with remote default
    default  merges with remote default
    octopus  merges with remote topic-a
                and with remote topic-b
                and with remote topic-c
    rebase  rebases onto remote default
  Local refs configured for 'git push':
    default pushes to default  (local out of date)
    default pushes to upstream (create)
* remote two
  Fetch URL: ../two
  Push  URL: ../three
  HEAD branch: default
  Local refs configured for 'git push':
    ahead   forces to default (fast-forwardable)
    default pushes to another (up to date)
EOF

test_expect_success 'show' '
	(
		cd test &&
		git config --add remote.origin.fetch refs/heads/default:refs/heads/upstream &&
		git fetch &&
		git checkout -b ahead origin/default &&
		echo 1 >>file &&
		test_tick &&
		git commit -m update file &&
		git checkout default &&
		git branch --track octopus origin/default &&
		git branch --track rebase origin/default &&
		git branch -d -r origin/default &&
		...
		git config --add remote.origin.push refs/heads/default:refs/heads/upstream &&
		git config --add remote.origin.push +refs/tags/lastbackup &&
		git config --add remote.two.push +refs/heads/ahead:refs/heads/default &&
		git config --add remote.two.push refs/heads/default:refs/heads/another &&
...
* remote origin
  ...
  Local refs configured for 'git push' (status not queried):
    ...
    refs/heads/default   pushes to refs/heads/upstream
...
test_expect_success 'set-head --auto has no problem w/multiple HEADs' '
	(
		...
		echo "two/HEAD set to default" >expect &&
		test_i18ncmp expect output
	)
'
...
test_expect_success 'fetch mirrors can prune' '
	(
		...
		test_must_fail git rev-parse --verify refs/heads/default
	)
'
...
test_expect_success 'rename a remote' '
	...
	(
		...
		test "$(git symbolic-ref refs/remotes/upstream/HEAD)" = "refs/remotes/upstream/default" &&
		test "$(git rev-parse upstream/default)" = "$(git rev-parse default)" &&
		test "$(git config remote.upstream.fetch)" = "+refs/heads/*:refs/remotes/upstream/*" &&
		test "$(git config branch.default.remote)" = "upstream" &&
		test "$(git config branch.default.pushRemote)" = "upstream" &&
...
test_expect_success 'rename does not update a non-default fetch refspec' '
	...
	(
		...
		git rev-parse -q origin/default
	)
'
...
test_expect_success 'rename a remote with name prefix of other remote' '
	...
	(
		...
		test "$(git rev-parse origin/default)" = "$(git rev-parse default)"
	)
'
...
test_expect_success 'remove a remote' '
	...
	(
		...
		test_must_fail git config branch.default.pushRemote &&
		test "$(git config --global remote.pushDefault)" = "origin"
	)
'
...
URL: $(pwd)/one
Push: refs/heads/default:refs/heads/upstream
Push: refs/heads/next:refs/heads/upstream2
Pull: refs/heads/default:refs/heads/origin
...
test_expect_success 'migrate a remote from named file in $GIT_DIR/remotes' '
	...
	(
		...
		refs/heads/default:refs/heads/upstream
		refs/heads/next:refs/heads/upstream2
		EOF
		cat >fetch_expected <<-\EOF &&
		refs/heads/default:refs/heads/origin
...
test_expect_success 'migrate a remote from named file in $GIT_DIR/branches' '
	...
	(
		...
		test "$(git config remote.origin.url)" = "$origin_url" &&
		test "$(git config remote.origin.fetch)" = "refs/heads/default:refs/heads/origin" &&
		test "$(git config remote.origin.push)" = "HEAD:refs/heads/default"
	)
'
...
test_expect_success 'remote set-branches' '
	...
	+refs/heads/default:refs/remotes/scratch/default
	...
	(
		...
		git remote set-branches scratch maint default next &&
		git config --get-all remote.scratch.fetch >config-result &&
...
test_expect_success 'remote set-branches with --mirror' '
	...
	echo "+refs/heads/default:refs/heads/default" >expect.replace &&
	...
	(
		...
		git remote set-branches scratch heads/default &&
...
test_extra_arg set-head origin default
...
test_expect_success 'unqualified <dst> refspec DWIM and advice' '
	...
	(
		cd test &&
		git tag -a -m "Some tag" some-tag default &&
		exit_with=true &&
...
test_expect_success 'refs/remotes/* <src> refspec and unqualified <dst> DWIM and advice' '
	(
		cd two &&
		git tag -a -m "Some tag" my-tag default &&
