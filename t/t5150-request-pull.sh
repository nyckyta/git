...
test_expect_success 'pull request after push' '
	...
	(
		...
		git checkout initial &&
		git merge --ff-only default &&
		git push origin default:for-upstream &&
		git request-pull initial origin default:for-upstream >../request
	) &&
...
test_expect_success 'request asks HEAD to be pulled' '
	...
	(
		...
		git checkout initial &&
		git merge --ff-only default &&
		git push --tags origin default simplify &&
		git push origin default:for-upstream &&
...
test_expect_success 'request-pull ignores OPTIONS_KEEPDASHDASH poison' '
	...
	(
		...
		git merge --ff-only default &&
		git push origin default:for-upstream &&
		git request-pull -- initial "$downstream_url" default:for-upstream >../request
	)
