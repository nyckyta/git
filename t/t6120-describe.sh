...
check_describe heads/default --all HEAD
...
commit #$i
EOF"
		test $i = 1 && echo "from refs/heads/default^0"
		i=$(($i + 1))
...
test_expect_success 'describe commits with disjoint bases' '
	...
	(
		...
		git tag B -a -m B &&
		git merge --no-ff --allow-unrelated-histories default -m x &&
