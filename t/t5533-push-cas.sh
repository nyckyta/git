...
test_expect_success 'push to update (protected)' '
	...
	(
		cd dst &&
		test_commit D &&
		test_must_fail git push --force-with-lease=default:default origin default 2>err &&
		grep "stale info" err
	) &&
	git ls-remote . refs/heads/default >expect &&
	git ls-remote src refs/heads/default >actual &&
...
test_expect_success 'push to update (protected, forced)' '
	...
	(
		cd dst &&
		test_commit D &&
		git push --force --force-with-lease=default:default origin default 2>err &&
		grep "forced update" err
	) &&
	git ls-remote dst refs/heads/default >expect &&
	git ls-remote src refs/heads/default >actual &&
	test_cmp expect actual
'
...
test_expect_success 'push to update (protected, tracking)' '
	...
	git ls-remote src refs/heads/default >expect &&
	(
		cd dst &&
		test_commit E &&
		git ls-remote . refs/remotes/origin/default >expect &&
		test_must_fail git push --force-with-lease=default origin default &&
		git ls-remote . refs/remotes/origin/default >actual &&
		test_cmp expect actual
	) &&
	git ls-remote src refs/heads/default >actual &&
	test_cmp expect actual
...
test_expect_success 'push to update (protected, tracking, forced)' '
	...
	(
		...
		test_commit E &&
		git ls-remote . refs/remotes/origin/default >expect &&
		git push --force --force-with-lease=default origin default
	) &&
	git ls-remote dst refs/heads/default >expect &&
	git ls-remote src refs/heads/default >actual &&
	test_cmp expect actual
...
test_expect_success 'push to update (allowed)' '
	...
	(
		cd dst &&
		test_commit D &&
		git push --force-with-lease=default:default^ origin default
	) &&
	git ls-remote dst refs/heads/default >expect &&
	git ls-remote src refs/heads/default >actual &&
	test_cmp expect actual
...
test_expect_success 'push to update (allowed, tracking)' '
	...
	(
		cd dst &&
		test_commit D &&
		git push --force-with-lease=default origin default 2>err &&
		! grep "forced update" err
	) &&
	git ls-remote dst refs/heads/default >expect &&
	git ls-remote src refs/heads/default >actual &&
...
test_expect_success 'push to delete (protected)' '
	...
	(
		cd dst &&
		test_must_fail git push --force-with-lease=default:default^ origin :default
	) &&
	git ls-remote src refs/heads/default >actual &&
	test_cmp expect actual
'
...
test_expect_success 'push to delete (protected, forced)' '
	...
	(
		cd dst &&
		git push --force --force-with-lease=default:default^ origin :default
	) &&
	git ls-remote src refs/heads/default >actual &&
...
test_expect_success 'cover everything with default force-with-lease (protected)' '
	...
	(
		cd src &&
		git branch naster default^
	) &&
	git ls-remote src refs/heads/\* >expect &&
	(
		cd dst &&
		test_must_fail git push --force-with-lease origin default default:naster
	) &&
...
test_expect_success 'cover everything with default force-with-lease (allowed)' '
	...
	(
		cd dst &&
		git fetch &&
		git push --force-with-lease origin default default:naster
	) &&
	git ls-remote dst refs/heads/default |
	sed -e "s/default/naster/" >expect &&
...
test_expect_success 'background updates of REMOTE can be mitigated with a non-updated REMOTE-push' '
	...
	(
		...
		git push origin-push default:default
	) &&
