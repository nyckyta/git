...
rm -f .git/logs/refs/heads/default
...
test_expect_success 'Query "default@{May 25 2005}" (before history)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{May 25 2005}" >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
	echo "warning: log for '\''default'\'' only goes back to $ed" >expect &&
	test_i18ncmp expect e
'
test_expect_success 'Query default@{2005-05-25} (before history)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify default@{2005-05-25} >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
	echo "warning: log for '\''default'\'' only goes back to $ed" >expect &&
	test_i18ncmp expect e
'
test_expect_success 'Query "default@{May 26 2005 23:31:59}" (1 second before history)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{May 26 2005 23:31:59}" >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
	echo "warning: log for '\''default'\'' only goes back to $ed" >expect &&
	test_i18ncmp expect e
'
test_expect_success 'Query "default@{May 26 2005 23:32:00}" (exactly history start)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{May 26 2005 23:32:00}" >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
	test_must_be_empty e
'
test_expect_success 'Query "default@{May 26 2005 23:32:30}" (first non-creation change)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{May 26 2005 23:32:30}" >o 2>e &&
	echo "$A" >expect &&
	test_cmp expect o &&
	test_must_be_empty e
'
test_expect_success 'Query "default@{2005-05-26 23:33:01}" (middle of history with gap)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{2005-05-26 23:33:01}" >o 2>e &&
	echo "$B" >expect &&
	test_cmp expect o &&
	test_i18ngrep -F "warning: log for ref $m has gap after $gd" e
'
test_expect_success 'Query "default@{2005-05-26 23:38:00}" (middle of history)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{2005-05-26 23:38:00}" >o 2>e &&
	echo "$Z" >expect &&
	test_cmp expect o &&
	test_must_be_empty e
'
test_expect_success 'Query "default@{2005-05-26 23:43:00}" (exact end of history)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{2005-05-26 23:43:00}" >o 2>e &&
	echo "$E" >expect &&
	test_cmp expect o &&
	test_must_be_empty e
'
test_expect_success 'Query "default@{2005-05-28}" (past end of history)' '
	test_when_finished "rm -f o e" &&
	git rev-parse --verify "default@{2005-05-28}" >o 2>e &&
	echo "$D" >expect &&
	test_cmp expect o &&
...
test_expect_success 'git cat-file blob default:F (expect OTHER)' '
	test OTHER = $(git cat-file blob default:F)
'
test_expect_success 'git cat-file blob default@{2005-05-26 23:30}:F (expect TEST)' '
	test TEST = $(git cat-file blob "default@{2005-05-26 23:30}:F")
'
test_expect_success 'git cat-file blob default@{2005-05-26 23:42}:F (expect OTHER)' '
	test OTHER = $(git cat-file blob "default@{2005-05-26 23:42}:F")
'
...
test_expect_success 'stdin fails on unbalanced quotes' '
	echo "create $a \"default" >stdin &&
	test_must_fail git update-ref --stdin <stdin 2>err &&
	grep "fatal: badly quoted argument: \\\"default" err
'

test_expect_success 'stdin fails on invalid escape' '
	echo "create $a \"def\zult\"" >stdin &&
	test_must_fail git update-ref --stdin <stdin 2>err &&
	grep "fatal: badly quoted argument: \\\"def\\\\zult\\\"" err
'

test_expect_success 'stdin fails on junk after quoted argument' '
	echo "create \"$a\"default" >stdin &&
	test_must_fail git update-ref --stdin <stdin 2>err &&
	grep "fatal: unexpected character after quoted argument: \\\"$a\\\"default" err
'
...
test_expect_success 'stdin succeeds with escaped character' '
	git update-ref -d $a &&
	echo "create $a \"defa\\165lt\"" >stdin &&
	git update-ref --stdin <stdin &&
	git rev-parse $m >expect &&
	git rev-parse $a >actual &&
	test_cmp expect actual
