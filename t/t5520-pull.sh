...
test_expect_success 'pulling into void using default:default' '
	git init cloned-uho &&
	(
		cd cloned-uho &&
		git pull .. default:default
	) &&
	test_path_is_file file &&
...
test_expect_success 'pulling into void does not overwrite untracked files' '
	...
	(
		cd cloned-untracked &&
		echo untracked >file &&
		test_must_fail git pull .. default &&
...
test_expect_success 'pulling into void must not create an octopus' '
	...
	(
		cd cloned-octopus &&
		test_must_fail git pull .. default default &&
		test_path_is_missing file
	)
'

test_expect_success 'test . as a remote' '
	git branch copy default &&
	git config branch.copy.remote . &&
	git config branch.copy.merge refs/heads/default &&
	echo updated >file &&
...
test_expect_success '--rebase fails with multiple branches' '
	git reset --hard before-rebase &&
	test_must_fail git pull --rebase . copy default 2>err &&
...
# add a feature branch, keep-merge, that is merged into default, so the
...
test_expect_success 'pull --rebase works on branch yet to be born' '
	git rev-parse default >expect &&
	mkdir empty_repo &&
	(
		cd empty_repo &&
		git init &&
		git pull --rebase .. default &&
...
test_expect_success 'pull --rebase fails on unborn branch with staged changes' '
	...
	(
		...
		git ls-files >actual &&
		test_cmp expect actual &&
		test_must_fail git pull --rebase .. default 2>err &&
		git ls-files >actual &&
		test_cmp expect actual &&
