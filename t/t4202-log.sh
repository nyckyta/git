...
test_expect_success 'log --raw --graph -m with merge' '
	git log --raw --graph --oneline -m default | head -n 500 >actual &&
	grep "initial" actual
'

test_expect_success 'diff-tree --graph' '
	git diff-tree --graph default^ | head -n 500 >actual &&
	grep "one" actual
'
...
| |\
| * \   Merge branch 'default' (early part) into tangle
| |\ \
| * | | tangle-a
* | | |   Merge branch 'side'
|\ \ \ \
...
test_expect_success GPG 'setup signed branch' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b signed default &&
	echo foo >foo &&
	git add foo &&
...
test_expect_success GPG 'setup signed branch with subkey' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b signed-subkey default &&
	echo foo >foo &&
	git add foo &&
...
test_expect_success GPGSM 'setup signed branch x509' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b signed-x509 default &&
	echo foo >foo &&
	git add foo &&
...
test_expect_success GPG 'log --graph --show-signature for merged tag' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b plain default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout -b tagged default &&
	echo bbb >baz &&
	git add baz &&
...
test_expect_success GPG 'log --graph --show-signature for merged tag in shallow clone' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b plain-shallow default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout --detach default &&
	echo bbb >baz &&
	git add baz &&
	git commit -m baz_commit &&
...
test_expect_success GPG 'log --graph --show-signature for merged tag with missing key' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b plain-nokey default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout -b tagged-nokey default &&
	echo bbb >baz &&
	git add baz &&
	git commit -m baz_commit &&
...
test_expect_success GPG 'log --graph --show-signature for merged tag with bad signature' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b plain-bad default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout -b tagged-bad default &&
	echo bbb >baz &&
	git add baz &&
	git commit -m baz_commit &&
...
test_expect_success GPG 'log --show-signature for merged tag with GPG failure' '
	test_when_finished "git reset --hard && git checkout default" &&
	git checkout -b plain-fail default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout -b tagged-fail default &&
	echo bbb >baz &&
	git add baz &&
	git commit -m baz_commit &&
...
test_expect_success GPGSM 'log --graph --show-signature for merged tag x509' '
	...
	test_config gpg.format x509 &&
	test_config user.signingkey $GIT_COMMITTER_EMAIL &&
	git checkout -b plain-x509 default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout -b tagged-x509 default &&
	echo bbb >baz &&
	git add baz &&
	git commit -m baz_commit &&
...
test_expect_success GPGSM 'log --graph --show-signature for merged tag x509 missing key' '
	...
	test_config gpg.format x509 &&
	test_config user.signingkey $GIT_COMMITTER_EMAIL &&
	git checkout -b plain-x509-nokey default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout -b tagged-x509-nokey default &&
	echo bbb >baz &&
	git add baz &&
	git commit -m baz_commit &&
...
test_expect_success GPGSM 'log --graph --show-signature for merged tag x509 bad signature' '
	...
	test_config gpg.format x509 &&
	test_config user.signingkey $GIT_COMMITTER_EMAIL &&
	git checkout -b plain-x509-bad default &&
	echo aaa >bar &&
	git add bar &&
	git commit -m bar_commit &&
	git checkout -b tagged-x509-bad default &&
	echo bbb >baz &&
	git add baz &&
	git commit -m baz_commit &&
...
test_expect_success 'log diagnoses bogus HEAD' '
	...
	echo 1234abcd >empty/.git/refs/heads/default &&
	test_must_fail git -C empty log 2>stderr &&
