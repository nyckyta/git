...
test_expect_success 'set up expected reflog' '
	cat >expect.all <<-EOF
	HEAD@{0} commit (merge): Merge branch ${SQ}default${SQ} into side
	HEAD@{1} commit: three
	HEAD@{2} checkout: moving from default to side
	HEAD@{3} commit: two
...
test_expect_success 'walking multiple reflogs shows all' '
	...
	#      we use "-k 1" to ensure that we see HEAD before default before
	#      side when breaking ties.
	{
		do_walk --date=unix HEAD &&
		do_walk --date=unix side &&
		do_walk --date=unix default
	} >expect.raw &&
	sort -t "{" -k 2nr -k 1 <expect.raw >expect &&
	do_walk --date=unix HEAD default side >actual &&
	test_cmp expect actual
'
