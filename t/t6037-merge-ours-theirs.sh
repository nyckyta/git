...
test_expect_success 'binary file with -Xours/-Xtheirs' '
	...
	git reset --hard default &&
	git merge -s recursive -X ours side &&
	git diff --exit-code default HEAD -- file
'

test_expect_success 'pull passes -X to underlying merge' '
	git reset --hard default && git pull -s recursive -Xours . side &&
	git reset --hard default && git pull -s recursive -X ours . side &&
	git reset --hard default && git pull -s recursive -Xtheirs . side &&
	git reset --hard default && git pull -s recursive -X theirs . side &&
	git reset --hard default && test_must_fail git pull -s recursive -X bork . side
'

test_expect_success SYMLINKS 'symlink with -Xours/-Xtheirs' '
	git reset --hard default &&
	git checkout -b two default &&
