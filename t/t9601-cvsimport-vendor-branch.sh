...
test_expect_success PERL 'check HEAD out of cvs repository' 'test_cvs_co default'

test_expect_success PERL 'check default out of git repository' 'test_git_co default'

test_expect_success PERL 'check a file that was imported once' '

	test_cmp_branch_file default imported-once.txt

'

test_expect_failure PERL 'check a file that was imported twice' '

	test_cmp_branch_file default imported-twice.txt

'

test_expect_success PERL 'check a file that was imported then modified on HEAD' '

	test_cmp_branch_file default imported-modified.txt

'

test_expect_success PERL 'check a file that was imported, modified, then imported again' '

	test_cmp_branch_file default imported-modified-imported.txt

'

test_expect_success PERL 'check a file that was added to HEAD then imported' '

	test_cmp_branch_file default added-imported.txt

'

test_expect_success PERL 'a vendor branch whose tag has been removed' '

	test_cmp_branch_file default imported-anonymously.txt
