...
add_upstream_commit() {
	(
		...
		echo "   $head1..$head2  default    -> origin/default" >> ../expect.err
	) &&
	(
		cd deepsubmodule &&
		head1=$(git rev-parse --short HEAD) &&
...
test_expect_success "Recursion stops when no new submodule commits are fetched" '
	...
	echo "   $head1..$head2  default    -> origin/default" >>expect.err.sub &&
...
test_expect_success "Recursion doesn't happen when new superproject commits don't change any submodules" '
	...
	echo "   $head1..$head2  default    -> origin/default" >> expect.err.file &&
	(
		cd downstream &&
		git fetch >../actual.out 2>../actual.err
	) &&
...
test_expect_success "Recursion picks up all submodules when necessary" '
	...
	echo "   $head1..$head2  default    -> origin/default" >> expect.err.2 &&
...
test_expect_success 'fetch new submodule commits on-demand without .gitmodules entry' '
	...
	git -C downstream checkout origin/default &&
