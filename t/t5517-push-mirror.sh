...
test_expect_success 'push mirror creates new branches' '
	...
	master_master=$(cd default && git show-ref -s --verify refs/heads/default) &&
	mirror_master=$(cd mirror && git show-ref -s --verify refs/heads/default) &&
...
test_expect_success 'push mirror adds, updates and removes branches together' '
	...
	(
		...
		git push --mirror up &&
		git branch -D remove &&
		git branch add default &&
		echo two >foo && git add foo && git commit -m two &&
		git push --mirror up
	) &&
	master_master=$(cd default && git show-ref -s --verify refs/heads/default) &&
	master_add=$(cd default && git show-ref -s --verify refs/heads/add) &&
	mirror_master=$(cd mirror && git show-ref -s --verify refs/heads/default) &&
...
test_expect_success 'push mirror creates new tags' '
	...
	(
		cd default &&
		echo one >foo && git add foo && git commit -m one &&
		git tag -f tmaster default &&
		git push --mirror up
	) &&
	master_master=$(cd default && git show-ref -s --verify refs/tags/tmaster) &&
...
test_expect_success 'push mirror adds, updates and removes tags together' '
	...
	(
		...
		git push --mirror up &&
		git tag -d tremove &&
		git tag tadd default &&
		echo two >foo && git add foo && git commit -m two &&
		git tag -f tmaster default &&
		git push --mirror up
	) &&
	master_master=$(cd default && git show-ref -s --verify refs/tags/tmaster) &&
	master_add=$(cd default && git show-ref -s --verify refs/tags/tadd) &&
	mirror_master=$(cd mirror && git show-ref -s --verify refs/tags/tmaster) &&
...
test_expect_success 'push to mirrored repository with refspec fails' '
	...
	(
		...
		test_must_fail git push up default
	)
'
