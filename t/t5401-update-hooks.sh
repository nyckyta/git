...
test_expect_success setup '
	...
	git update-ref refs/heads/default $commit0 &&
	git update-ref refs/heads/tofail $commit1 &&
	git clone --bare ./. victim.git &&
	GIT_DIR=victim.git git update-ref refs/heads/tofail $commit1 &&
	git update-ref refs/heads/default $commit1 &&
...
test "$1" = refs/heads/default || exit
EOF
...
test_expect_success push '
	test_must_fail git send-pack --force ./victim.git \
		default tofail >send.out 2>send.err
'

test_expect_success 'updated as expected' '
	test $(GIT_DIR=victim.git git rev-parse default) = $commit1 &&
...
test_expect_success 'pre-receive hook input' '
	(echo $commit0 $commit1 refs/heads/default &&
	 echo $commit1 $commit0 refs/heads/tofail
	) | test_cmp - victim.git/pre-receive.stdin
'

test_expect_success 'update hook arguments' '
	(echo refs/heads/default $commit0 $commit1 &&
...
remote: STDOUT update refs/heads/default
remote: STDERR update refs/heads/default
...
test_expect_success 'pre-receive hook that forgets to read its input' '
	...
	do
		git branch branch_$v default || return
	done &&
