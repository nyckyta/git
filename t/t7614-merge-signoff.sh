...
Merge branch 'default' into other-branch
...
EOF
	...
	# Initial commit and feature branch to merge default into it.
...
test_expect_success 'git merge --signoff adds a sign-off line' '
	...
	test_commit default-branch-2 file2 2 &&
	git checkout other-branch &&
	git merge default --signoff --no-edit &&
...
test_expect_success 'git merge does not add a sign-off line' '
	git checkout default &&
	test_commit default-branch-3 file3 3 &&
	git checkout other-branch &&
...
test_expect_success 'git merge --no-signoff flag cancels --signoff flag' '
	git checkout default &&
	test_commit default-branch-4 file4 4 &&
	git checkout other-branch &&
	git merge default --no-edit --signoff --no-signoff &&
	git cat-file commit HEAD | sed -e "1,/^\$/d" >actual &&
	test_cmp expected-unsigned actual
'
