...
test_expect_success 'refuse pushing rewound head without --force' '
	pushed_head=$(git rev-parse --verify default) &&
	victim_orig=$(cd victim && git rev-parse --verify default) &&
	test_must_fail git send-pack ./victim default &&
	victim_head=$(cd victim && git rev-parse --verify default) &&
	test "$victim_head" = "$victim_orig" &&
	# this should update
	git send-pack --force ./victim default &&
	victim_head=$(cd victim && git rev-parse --verify default) &&
	test "$victim_head" = "$pushed_head"
'

test_expect_success 'push can be used to delete a ref' '
	( cd victim && git branch extra default ) &&
	git send-pack ./victim :extra default &&
	( cd victim &&
...
test_expect_success 'refuse deleting push with denyDeletes' '
	(
	    ...
	    git branch extra default
	) &&
	test_must_fail git send-pack ./victim :extra default
'
...
test_expect_success 'cannot override denyDeletes with git -c send-pack' '
	...
	test_must_fail git -c receive.denyDeletes=false \
					send-pack ./victim :extra default
'
...
test_expect_success 'denyNonFastforwards trumps --force' '
	...
	victim_orig=$(cd victim && git rev-parse --verify default) &&
	test_must_fail git send-pack --force ./victim default^:default &&
	victim_head=$(cd victim && git rev-parse --verify default) &&
...
test_expect_success 'pushing explicit refspecs respects forcing' '
	rewound_push_setup &&
	parent_orig=$(cd parent && git rev-parse --verify default) &&
	(
	    cd child &&
	    test_must_fail git send-pack ../parent \
		refs/heads/default:refs/heads/default
	) &&
	parent_head=$(cd parent && git rev-parse --verify default) &&
	...
	parent_head=$(cd parent && git rev-parse --verify default) &&
	child_head=$(cd child && git rev-parse --verify default) &&
...
test_expect_success 'deny pushing to delete current branch' '
	...
	(
	    cd child &&
	    test_must_fail git send-pack ../parent :refs/heads/default 2>errs
	)
'
...
test_expect_success 'receive-pack de-dupes .have lines' '
	...
	cat >expect <<-EOF &&
	$local refs/heads/default
	$local refs/remotes/origin/HEAD
	$local refs/remotes/origin/default
	$shared .have
	EOF
