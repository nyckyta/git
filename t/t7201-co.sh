...
Creates default, forks renamer and side branches from it.
Test switching across them.

  ! [default] Initial A one, A two
...
test_expect_success "checkout from non-existing branch" '
	...
	git checkout -b delete-me default &&
	git update-ref -d --no-deref refs/heads/delete-me &&
	test refs/heads/delete-me = "$(git symbolic-ref HEAD)" &&
	git checkout default &&
	test refs/heads/default = "$(git symbolic-ref HEAD)"
'
...
test_expect_success "checkout -m with dirty tree" '
	...
	fill "M	one" "A	three" "D	two" >expect.default &&
	git diff --name-status default >current.default &&
	test_cmp expect.default current.default &&
...
test_expect_success "checkout -m with dirty tree, renamed" '
	...
	git checkout -f default && git clean -f &&
...
test_expect_success 'checkout --merge --conflict=diff3 <branch>' '
	...
	git checkout -f default && git reset --hard && git clean -f &&
...
test_expect_success 'checkout to detach HEAD (with advice declined)' '
	...
	M=$(git show-ref -s --verify refs/heads/default) &&
	test "z$H" = "z$M" &&
...
test_expect_success 'checkout w/--track from non-branch HEAD fails' '
    ...
    test_must_fail git symbolic-ref HEAD &&
    test "z$(git rev-parse default^0)" = "z$(git rev-parse HEAD)"
'
...
test_expect_success 'detach a symbolic link HEAD' '
    ...
    test "z$it" = zrefs/heads/default &&
    here=$(git rev-parse --verify refs/heads/default) &&
    git checkout side^ &&
    test "z$(git rev-parse --verify refs/heads/default)" = "z$here"
'
...
test_expect_success \
    ...
    git checkout default && git branch -D koala/bear &&
...
test_expect_success 'failing checkout -b should not break working tree' '
	...
	test $(git symbolic-ref HEAD) = refs/heads/default &&
	git diff --exit-code &&
