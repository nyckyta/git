...
test_expect_success setup '
	...
	git update-ref refs/remotes/origin/default default &&
...
test_atom head refname refs/heads/default
test_atom head refname: refs/heads/default
test_atom head refname:short default
test_atom head refname:lstrip=1 heads/default
test_atom head refname:lstrip=2 default
test_atom head refname:lstrip=-1 default
test_atom head refname:lstrip=-2 heads/default
test_atom head refname:rstrip=1 refs/heads
test_atom head refname:rstrip=2 refs
test_atom head refname:rstrip=-1 refs
test_atom head refname:rstrip=-2 refs/heads
test_atom head refname:strip=1 heads/default
test_atom head refname:strip=2 default
test_atom head refname:strip=-1 default
test_atom head refname:strip=-2 heads/default
test_atom head upstream refs/remotes/origin/default
test_atom head upstream:short origin/default
test_atom head upstream:lstrip=2 origin/default
test_atom head upstream:lstrip=-2 origin/default
test_atom head upstream:rstrip=2 refs/remotes
test_atom head upstream:rstrip=-2 refs/remotes
test_atom head upstream:strip=2 origin/default
test_atom head upstream:strip=-2 origin/default
test_atom head push refs/remotes/myfork/default
test_atom head push:short myfork/default
test_atom head push:lstrip=1 remotes/myfork/default
test_atom head push:lstrip=-1 default
test_atom head push:rstrip=1 refs/remotes/myfork
test_atom head push:rstrip=-1 refs
test_atom head push:strip=1 remotes/myfork/default
test_atom head push:strip=-1 default
test_atom head objecttype commit
test_atom head objectsize $((131 + hexlen))
test_atom head objectsize:disk $disklen
test_atom head deltabase $ZERO_OID
test_atom head objectname $(git rev-parse refs/heads/default)
test_atom head objectname:short $(git rev-parse --short refs/heads/default)
test_atom head objectname:short=1 $(git rev-parse --short=1 refs/heads/default)
test_atom head objectname:short=10 $(git rev-parse --short=10 refs/heads/default)
test_atom head tree $(git rev-parse refs/heads/default^{tree})
...
test_date () {
	...
	tagger_date=$4 &&
	cat >expected <<-EOF &&
	'refs/heads/default' '$committer_date' '$author_date'
...
test_expect_success 'setup for push:track[short]' '
	test_commit third &&
	git update-ref refs/remotes/myfork/default default &&
	git reset default~1
'
...
test_expect_success 'Check that :track[short] works when upstream is invalid' '
	...
	EOF
	test_when_finished "git config branch.default.merge refs/heads/default" &&
	git config branch.default.merge refs/heads/does-not-exist &&
	git for-each-ref \
...
test_expect_success 'set up color tests' '
	...
	$(git rev-parse --short refs/heads/default) <GREEN>default<RESET>
	$(git rev-parse --short refs/remotes/myfork/default) <GREEN>myfork/default<RESET>
	$(git rev-parse --short refs/remotes/origin/default) <GREEN>origin/default<RESET>
...
test_expect_success 'Check ambiguous head and tag refs (strict)' '
	...
	git commit -m "Branch" &&
	setdate_and_increment &&
	git tag -m "Tagging at $datestamp" default &&
	git for-each-ref --format "%(refname:short)" refs/heads/default refs/tags/default >actual &&
	test_cmp expected actual
'
...
test_expect_success '%(trailers:unfold) unfolds trailers' '
	git for-each-ref --format="%(trailers:unfold)" refs/heads/default >actual &&
	{
		unfold <trailers
		echo
	} >expect &&
...
test_expect_success '%(trailers:only) shows only "key: value" trailers' '
	git for-each-ref --format="%(trailers:only)" refs/heads/default >actual &&
	{
...
test_expect_success '%(trailers:only) and %(trailers:unfold) work together' '
	git for-each-ref --format="%(trailers:only,unfold)" refs/heads/default >actual &&
	git for-each-ref --format="%(trailers:unfold,only)" refs/heads/default >reverse &&
...
test_expect_success '%(contents:trailers:unfold) unfolds trailers' '
	git for-each-ref --format="%(contents:trailers:unfold)" refs/heads/default >actual &&
	{
		unfold <trailers
		echo
	} >expect &&
...
test_expect_success '%(contents:trailers:only) shows only "key: value" trailers' '
	git for-each-ref --format="%(contents:trailers:only)" refs/heads/default >actual &&
	{
		grep -v patch.description <trailers &&
		echo
	} >expect &&
...
test_expect_success '%(contents:trailers:only) and %(contents:trailers:unfold) work together' '
	git for-each-ref --format="%(contents:trailers:only,unfold)" refs/heads/default >actual &&
	git for-each-ref --format="%(contents:trailers:unfold,only)" refs/heads/default >reverse &&
	test_cmp actual reverse &&
	{
		grep -v patch.description <trailers | unfold &&
		echo
...
test_expect_success 'basic atom: head contents:trailers' '
	git for-each-ref --format="%(contents:trailers)" refs/heads/default >actual &&
	sanitize_pgp <actual >actual.clean &&
...
test_expect_success 'trailer parsing not fooled by --- line' '
	...
	git for-each-ref --format="%(trailers)" refs/heads/default >actual &&
	test_cmp expect actual
'

test_expect_success 'Add symbolic ref for the following tests' '
	git symbolic-ref refs/heads/sym refs/heads/default
'
...
test_expect_success ':remotename and :remoteref' '
	...
	(
		...
		git config branch.default.remote from &&
		git config branch.default.merge refs/heads/stable &&
		git remote add to southridge.audio:repo &&
		git config remote.to.push "refs/heads/*:refs/heads/pushed/*" &&
		git config branch.default.pushRemote to &&
		for pair in "%(upstream)=refs/remotes/from/stable" \
			"%(upstream:remotename)=from" \
			"%(upstream:remoteref)=refs/heads/stable" \
			"%(push)=refs/remotes/to/pushed/default" \
			"%(push:remotename)=to" \
			"%(push:remoteref)=refs/heads/pushed/default"
		do
			echo "${pair#*=}" >expect &&
			git for-each-ref --format="${pair%=*}" \
				refs/heads/default >actual &&
			test_cmp expect actual
		done &&
...
test_expect_success 'for-each-ref --ignore-case ignores case' '
	git for-each-ref --format="%(refname)" refs/heads/DEFAULT >actual &&
