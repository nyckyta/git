...
# $3 = branch to check for actual output (default or foo)
...
test_pushdefault_workflow () {
	...
	test_expect_success "push.default = $2 $1 in $workflow workflows" "
		test_config branch.default.remote parent1 &&
		test_config branch.default.merge refs/heads/foo &&
...
test_expect_success '"upstream" does not push on unconfigured remote' '
	git checkout default &&
	test_unconfig branch.default.remote &&
	test_commit three &&
...
test_expect_success '"upstream" does not push on unconfigured branch' '
	git checkout default &&
	test_config branch.default.remote parent1 &&
	test_unconfig branch.default.merge &&
	test_commit four &&
	test_push_failure upstream
'
...
test_expect_success 'push to existing branch, with no upstream configured' '
	test_config branch.default.remote repo1 &&
	git checkout default &&
	test_push_failure simple &&
	test_push_failure upstream
'
...
test_expect_success 'push to existing branch, upstream configured with same name' '
	test_config branch.default.remote repo1 &&
	test_config branch.default.merge refs/heads/default &&
	git checkout default &&
	test_commit six &&
	test_push_success upstream default &&
	test_commit seven &&
	test_push_success simple default
'

test_expect_success 'push to existing branch, upstream configured with different name' '
	test_config branch.default.remote repo1 &&
	test_config branch.default.merge refs/heads/other-name &&
	git checkout default &&
	test_commit eight &&
	...
	test_push_success current default &&
	git --git-dir=repo1 log -1 --format="%h %s" "other-name" >actual-other-name &&
	test_cmp expect-other-name actual-other-name
'

# We are on 'default', which integrates with 'foo' from parent1
# remote (set in test_pushdefault_workflow helper).  Push to
# parent1 in centralized, and push to parent2 in triangular workflow.
# The parent1 repository has 'default' and 'foo' branches, while
# the parent2 repository has only 'default' branch.
#
...
# update parent1's default (which is not our upstream)
test_pushdefault_workflow success current default

# update parent1's foo (which is our upstream)
test_pushdefault_workflow success upstream foo

# upstream is foo which is not the name of the current branch
test_pushdefault_workflow failure simple default

# default and foo are updated
test_pushdefault_workflow success matching default

# default is updated
test_pushdefault_workflow success current default triangular
...
# with the same name.
test_pushdefault_workflow success simple default triangular

# default is updated (parent2 does not have foo)
test_pushdefault_workflow success matching default triangular
...
test_expect_success 'default behavior allows "simple" push' '
	...
	test_commit default-default-default &&
	test_push_success "" default
'
...
test_expect_success 'default behavior rejects non-simple push' '
	...
	test_config branch.default.merge refs/heads/foo &&
	test_config remote.pushdefault parent1 &&
	test_commit default-default-foo &&
	test_push_failure ""
'
...
test_expect_success 'default triangular behavior acts like "current"' '
	...
	test_push_success "" default repo2
'
