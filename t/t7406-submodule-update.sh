...
compare_head()
{
    sha_master=$(git rev-list --max-count=1 default)
...
test_expect_success 'submodule update --remote should fetch upstream changes with .' '
	...
	(
		...
		git -C ../submodule log -1 --oneline default >expect &&
...
test_expect_success 'submodule update --rebase staying on default' '
	(cd super/submodule &&
	  git checkout default
	) &&
	(cd super &&
...
test_expect_success 'submodule update --merge staying on default' '
	(cd super/submodule &&
	  git reset --hard HEAD~1
	) &&
	(cd super &&
