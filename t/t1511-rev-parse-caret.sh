...
test_expect_success 'ref^{/non-existent}' '
	test_must_fail git rev-parse default^{/non-existent}
'

test_expect_success 'ref^{/Initial}' '
	git rev-parse ref >expected &&
	git rev-parse default^{/Initial} >actual &&
	test_cmp expected actual
'

test_expect_success 'ref^{/!Exp}' '
	test_must_fail git rev-parse default^{/!Exp}
'

test_expect_success 'ref^{/!}' '
	test_must_fail git rev-parse default^{/!}
'

test_expect_success 'ref^{/!!Exp}' '
	git rev-parse expref >expected &&
	git rev-parse default^{/!!Exp} >actual &&
	test_cmp expected actual
'
...
test_expect_success 'ref^{/!-non-existent}' '
	git rev-parse default >expected &&
	git rev-parse default^{/!-non-existent} >actual &&
	test_cmp expected actual
'

test_expect_success 'ref^{/!-Changed}' '
	git rev-parse expref >expected &&
	git rev-parse default^{/!-Changed} >actual &&
	test_cmp expected actual
'
