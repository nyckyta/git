...
test_http_push_nonff "$HTTPD_DOCUMENT_ROOT_PATH"/test_repo.git \
	"$ROOT_PATH"/test_repo_clone default 		success

test_expect_success 'push fails for non-fast-forward refs unmatched by remote helper' '
	# create a dissimilarly-named remote ref so that git is unable to match the
	# two refs (viz. local, remote) unless an explicit refspec is provided.
	git push origin default:retsam &&

	echo "change changed" > path2 &&
	git commit -a -m path2 --amend &&

	# push default too; this ensures there is at least one '"'push'"' command to
	# the remote helper and triggers interaction with the helper.
	test_must_fail git push -v origin +default default:retsam >output 2>&1'

test_expect_success 'push fails for non-fast-forward refs unmatched by remote helper: remote output' '
	grep "^ + [a-f0-9]*\.\.\.[a-f0-9]* *default -> default (forced update)$" output &&
	grep "^ ! \[rejected\] *default -> retsam (non-fast-forward)$" output
'
...
## References of remote: atomic1(1)            default(2) collateral(2) other(2)
## References of local :            atomic2(2) default(1) collateral(3) other(2) collateral1(3) atomic(1)
## Atomic push         :                       default(1) collateral(3)                         atomic(1)
test_expect_success 'push --atomic also prevents branch creation, reports collateral' '
	...
	git branch other &&
	git push "$up" atomic1 default collateral other &&
	git tag -d atomic1 &&
	...
	# Make default incompatible with upstream to provoke atomic
	git checkout default &&
	git reset --hard HEAD^ &&
	...
	test_must_fail git push --atomic "$up" default atomic collateral 2>output &&
	...
	# on default...
	git -C "$d" rev-parse refs/heads/default >actual &&
	...
	grep "^ ! .*rejected.* default -> default" output &&
...
test_expect_success 'colorize errors/hints' '
	...
	test_must_fail git -c color.transport=always -c color.advice=always \
		-c color.push=always \
		push origin origin/default^:default 2>act &&
