...
packs in the repository `default.git`. The relationship between packs (P1-P8)
...
(marked with letter s) are shared through alt-odb (of `default.git`). The
...
test_expect_success 'setup default repo' '
...
#
#############################################################################
test_expect_success 'default: no redundant for pack 1, 2, 3' '
...
#
#############################################################################
test_expect_success 'default: one of pack-2/pack-3 is redundant' '
...
#
#############################################################################
test_expect_success 'default: pack 2, 4, and 6 are redundant' '
...
#
#############################################################################
test_expect_success 'default: pack-8 (subset of pack-1) is also redundant' '
...
test_expect_success 'default: clean loose objects' '
	(
		cd "$master_repo" &&
		git prune-packed &&
...
test_expect_success 'default: remove redundant packs and pass fsck' '
	(
		cd "$master_repo" &&
