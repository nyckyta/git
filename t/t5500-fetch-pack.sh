...
test_expect_success 'in_vain resetted upon ACK' '
	...
	# Linked list of commits on default. The first is common; the rest are
	# not.
	...
	# default. The first is common; the rest are not.
	...
	# The new commit that the client wants to fetch.
	git -C myserver checkout default &&
	...
	git -C myclient fetch --progress origin default 2>log &&
...
test_expect_success 'shallow fetch with tags does not break the repository' '
	...
	(
		...
		cd repo2 &&
		git init &&
		git fetch --depth=2 ../.git default:branch &&
		git fsck
	)
'
...
test_expect_success 'fetch-pack can fetch a raw sha1 that is advertised as a ref' '
	...
	git -C client fetch-pack ../server \
		$(git -C server rev-parse refs/heads/default)
'
...
test_expect_success 'fetch-pack cannot fetch a raw sha1 that is not advertised as a ref' '
	...
	test_must_fail env GIT_TEST_PROTOCOL_VERSION=0 git -C client fetch-pack ../server \
		$(git -C server rev-parse refs/heads/default^) 2>err &&
...
test_expect_success 'fetch shallow since ...' '
	...
	git -C shallow11 log --pretty=tformat:%s origin/default >actual &&
	cat >expected <<-\EOF &&
	three
	two
	EOF
...
test_expect_success 'shallow since with commit graph and already-seen commit' '
	...
	0032have $(git rev-parse default)
	0000
	EOF
	)
'
...
test_expect_success 'fetch exclude tag one' '
	...
	git -C shallow12 log --pretty=tformat:%s origin/default >actual &&
...
test_expect_success 'fetching deepen' '
	...
	test_commit four &&
	git -C deepen log --pretty=tformat:%s default >actual &&
	echo three >expected &&
	test_cmp expected actual &&
	git -C deepen fetch --deepen=1 &&
	git -C deepen log --pretty=tformat:%s origin/default >actual &&
	cat >expected <<-\EOF &&
	four
	three
	two
