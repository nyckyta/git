...
test_expect_success "clone and setup child repos" '
	...
	(
		cd two &&
		git config branch.default.remote one &&
		git config remote.one.url ../one/.git/ &&
		git config remote.one.fetch refs/heads/default:refs/heads/one
	) &&
	git clone . three &&
	(
		cd three &&
		git config branch.default.remote two &&
		git config branch.default.merge refs/heads/one &&
		mkdir -p .git/remotes &&
		{
			echo "URL: ../two/.git/"
			echo "Pull: refs/heads/default:refs/heads/two"
...
test_expect_success "fetch test" '
	...
	his=$(cd ../one && git rev-parse refs/heads/default) &&
	test "z$mine" = "z$his"
'
...
test_expect_success "fetch test for-merge" '
	...
	git rev-parse --verify refs/heads/one &&
	master_in_two=$(cd ../two && git rev-parse default) &&
...
test_expect_success 'fetch --prune on its own works as expected' '
	...
	git clone . prune &&
	cd prune &&
	git update-ref refs/remotes/origin/extrabranch default &&
...
test_expect_success 'fetch --prune with a branch name keeps branches' '
	...
	git fetch --prune origin default &&
...
test_expect_success 'fetch --prune handles overlapping refspecs' '
	cd "$D" &&
	git update-ref refs/pull/42/head default &&
...
test_expect_success 'fetch --prune --tags prunes branches but not tags' '
	...
	cd prune-tags &&
	git tag sometag default &&
	# Create what looks like a remote-tracking branch from an earlier
	# fetch that has since been deleted from the remote:
	git update-ref refs/remotes/origin/fake-remote default &&
...
test_expect_success 'fetch --prune --tags with branch does not prune other things' '
	...
	git fetch --prune --tags origin default &&
	git rev-parse origin/extrabranch &&
	git rev-parse sometag
'
...
test_expect_success 'fetch --prune --tags with refspec prunes based on refspec' '
	...
	git tag sometag default &&
	git update-ref refs/remotes/origin/foo/otherbranch default &&
	git update-ref refs/remotes/origin/extrabranch default &&
...
test_expect_success '--refmap="" ignores configured refspec' '
	...
	git -C remote-refs rev-parse remotes/origin/default >old &&
	git -C remote-refs update-ref refs/remotes/origin/default default~1 &&
	git -C remote-refs rev-parse remotes/origin/default >new &&
	git -C remote-refs fetch --refmap= origin "+refs/heads/*:refs/hidden/origin/*" &&
	git -C remote-refs rev-parse remotes/origin/default >actual &&
	test_cmp new actual &&
...
test_expect_success '--refmap="" and --prune' '
	git -C remote-refs update-ref refs/remotes/origin/foo/otherbranch default &&
	git -C remote-refs update-ref refs/hidden/foo/otherbranch default &&
...
test_expect_success 'create bundle 1' '
	...
	git commit -a -m "tip" &&
	git bundle create bundle1 default^..default
'
...
test_expect_success 'create bundle 2' '
	cd "$D" &&
	git bundle create bundle2 default~2..default
'

test_expect_success 'unbundle 1' '
	cd "$D/bundle" &&
	git checkout -b some-branch &&
	test_must_fail git fetch "$D/bundle1" default:default
'
...
test_expect_success 'unbundle 2' '
	cd "$D/bundle" &&
	git fetch ../bundle2 default:default &&
	test "tip" = "$(git log -1 --pretty=oneline default | cut -d" " -f2)"
'
...
test_expect_success 'bundle should be able to create a full history' '
	...
	cd "$D" &&
	git tag -a -m '1.0' v1.0 default &&
	git bundle create bundle4 v1.0

'

test_expect_success 'fetch with a non-applying branch.<name>.merge' '
	git config branch.default.remote yeti &&
	git config branch.default.merge refs/heads/bigfoot &&
...
test_expect_success 'fetch from GIT URL with a non-applying branch.<name>.merge [2]' '
	...
	git config branch.default.remote blub &&
	git config branch.default.merge "$one_ref" &&
	git update-ref -d FETCH_HEAD &&
	git fetch one &&
	test $one_head = "$(git rev-parse --verify FETCH_HEAD)" &&
	test $this_head = "$(git rev-parse --verify HEAD)"
...
test_expect_success 'fetch from GIT URL with a non-applying branch.<name>.merge [3]' '
	git config branch.default.merge "${one_ref}_not" &&
	git update-ref -d FETCH_HEAD &&
	git fetch one &&
	test $one_head = "$(git rev-parse --verify FETCH_HEAD)" &&
	test $this_head = "$(git rev-parse --verify HEAD)"
...
test_expect_success 'bundle should record HEAD correctly' '
	...
	cd "$D" &&
	git bundle create bundle5 HEAD default &&
	git bundle list-heads bundle5 >actual &&
	for h in HEAD refs/heads/default
	do
...
test_expect_success 'mark initial state of origin/default' '
	(
		cd three &&
		git tag base-origin-default refs/remotes/origin/default
	)
'
...
test_expect_success 'explicit fetch should update tracking' '
	...
	(
		cd three &&
		git update-ref refs/remotes/origin/default base-origin-default &&
		o=$(git rev-parse --verify refs/remotes/origin/default) &&
		git fetch origin default &&
		n=$(git rev-parse --verify refs/remotes/origin/default) &&
		test "$o" != "$n" &&
...
test_expect_success 'explicit --refmap option overrides remote.*.fetch' '
	...
	(
		...
		git update-ref refs/remotes/origin/default base-origin-default &&
		o=$(git rev-parse --verify refs/remotes/origin/default) &&
		git fetch --refmap="refs/heads/*:refs/remotes/other/*" origin default &&
		n=$(git rev-parse --verify refs/remotes/origin/default) &&
		test "$o" = "$n" &&
		test_must_fail git rev-parse --verify refs/remotes/origin/side &&
		git rev-parse --verify refs/remotes/other/default
	)
'
...
test_expect_success 'explicitly empty --refmap option disables remote.*.fetch' '
	...
	(
		...
		git update-ref refs/remotes/origin/default base-origin-default &&
		o=$(git rev-parse --verify refs/remotes/origin/default) &&
		git fetch --refmap="" origin default &&
		n=$(git rev-parse --verify refs/remotes/origin/default) &&
		test "$o" = "$n" &&
		test_must_fail git rev-parse --verify refs/remotes/origin/side
	)
...
test_expect_success 'refuse to fetch into the current branch' '
	...
	test_must_fail git fetch . side:default

'

test_expect_success 'fetch into the current branch with --update-head-ok' '

	git fetch --update-head-ok . side:default
...
test_expect_success "should be able to fetch with duplicate refspecs" '
	...
	(
		cd dups &&
		git init &&
		git config branch.default.remote three &&
...
test_expect_success 'all boundary commits are excluded' '
	...
	git bundle create twoside-boundary.bdl default --since="$ad" &&
