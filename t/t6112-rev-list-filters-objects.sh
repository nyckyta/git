...
test_expect_success 'verify sparse:oid=oid-ish omits top-level files' '
	...
	git -C r3 rev-list --quiet --objects --filter-print-omitted \
		--filter=sparse:oid=default:pattern HEAD >revs &&
	awk -f print_1.awk revs |
	sed "s/~//" |
	sort >observed &&
...
test_expect_success 'combine:... with more than two sub-filters' '
	git -C r3 rev-list --objects \
		--filter=combine:tree:3+blob:limit=40+sparse:oid=default:pattern \
		HEAD >actual &&
	...
	git -C r3 rev-list --objects \
		--filter=combine:tree:3+blob:limit=40+sparse:oid=default:pattern1%2brenamed%25 \
		HEAD >actual &&
	...
	GIT_TRACE=$(pwd)/trace1 git -C r3 rev-list --objects \
		...
		--filter=sparse:oid="default:p;at%ter+n" \
		HEAD >actual &&

	test_cmp expect actual &&
	grep "Add to combine filter-spec: sparse:oid=default:p%3bat%25ter%2bn" \
		trace1 &&
	...
	GIT_TRACE=$(pwd)/trace2 git -C r3 rev-list --objects \
		--filter=sparse:oid=default:^~pattern \
		--filter=tree:3 --filter=blob:limit=40 \
		HEAD >actual &&

	test_cmp expect actual &&
	grep "Add to combine filter-spec: sparse:oid=default:%5e%7epattern" \
		trace2
'
