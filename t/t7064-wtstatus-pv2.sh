...
test_expect_success 'verify upstream fields in branch header' '
	...
	git clone . sub_repo &&
	(
		## Confirm local default tracks remote default.
		cd sub_repo &&
		...
		# branch.oid $HUF
		# branch.head default
		# branch.upstream origin/default
		# branch.ab +0 -0
		EOF
		...
		## Test upstream-gone case. Fake this by pointing origin/default at
		## a non-existing commit.
		OLD=$(git rev-parse origin/default) &&
		NEW=$ZERO_OID &&
