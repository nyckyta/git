...
test_expect_success 'two-way setup' '
	...
	echo file2 is not tracked on the default anymore &&
	rm -f file2 subdir/file2 &&
	git update-index --remove file2 subdir/file2 &&
	git commit -a -m "default removes file2 and subdir/file2"
'

test_expect_success 'two-way not clobbering' '

	echo >file2 default creates untracked file2 &&
	echo >subdir/file2 default creates untracked subdir/file2 &&
	if err=$(read_tree_u_must_succeed -m -u default side 2>&1)
	then
...
test_expect_success 'two-way with incorrect --exclude-per-directory (1)' '
	...
	if err=$(read_tree_u_must_succeed -m --exclude-per-directory=.gitignore default side 2>&1)
	then
		echo should have complained
		false
	else
...
test_expect_success 'two-way with incorrect --exclude-per-directory (2)' '
	...
	if err=$(read_tree_u_must_succeed -m -u --exclude-per-directory=foo --exclude-per-directory=.gitignore default side 2>&1)
	then
		echo should have complained
		false
	else
...
test_expect_success 'two-way clobbering a ignored file' '
	...
	read_tree_u_must_succeed -m -u --exclude-per-directory=.gitignore default side
'
...
test_expect_success 'three-way not complaining on an untracked path in both' '
	...
	echo >file2 file two is untracked on the default side &&
	echo >subdir/file2 file two is untracked on the default side &&

	read_tree_u_must_succeed -m -u branch-point default side
'
...
test_expect_success 'three-way not clobbering a working tree file' '
	...
	git checkout default &&
	echo >file3 file three created in default, untracked &&
	echo >subdir/file3 file three created in default, untracked &&
	if err=$(read_tree_u_must_succeed -m -u branch-point default side 2>&1)
	then
		echo should have complained
		false
	else
...
test_expect_success 'three-way not complaining on an untracked file' '
	...
	read_tree_u_must_succeed -m -u --exclude-per-directory=.gitignore branch-point default side
'
