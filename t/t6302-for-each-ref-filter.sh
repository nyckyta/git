...
test_expect_success 'setup some history and refs' '
	...
	git update-ref refs/odd/spot default
'
...
test_expect_success 'filtering with --points-at' '
	...
	refs/tags/three
	EOF
	git for-each-ref --format="%(refname)" --points-at=default >actual &&
...
test_expect_success 'filtering with --merged' '
	...
	refs/tags/two
	EOF
	git for-each-ref --format="%(refname)" --merged=default >actual &&
	test_cmp expect actual
'
...
test_expect_success 'filtering with --no-merged' '
	...
	EOF
	git for-each-ref --format="%(refname)" --no-merged=default >actual &&
	test_cmp expect actual
'
...
test_expect_success 'left alignment is default' '
	cat >expect <<-\EOF &&
	refname is refs/heads/default |refs/heads/default
...
test_expect_success 'check %(if)...%(then)...%(end) atoms' '
	...
	cat >expect <<-\EOF &&
	refs/heads/default Author: A U Thor
...
test_expect_success 'check %(if:equals=<string>)' '
	git for-each-ref --format="%(if:equals=default)%(refname:short)%(then)Found default%(else)Not default%(end)" refs/heads/ >actual &&
	cat >expect <<-\EOF &&
	Found default
	Not default
	EOF
...
test_expect_success 'check %(if:notequals=<string>)' '
	git for-each-ref --format="%(if:notequals=default)%(refname:short)%(then)Not default%(else)Found default%(end)" refs/heads/ >actual &&
	cat >expect <<-\EOF &&
	Found default
	Not default
	EOF
...
test_expect_success 'validate worktree atom' '
	...
	EOF
	git worktree add -b master_worktree worktree_dir default &&
