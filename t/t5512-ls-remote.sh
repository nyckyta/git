...
test_expect_success 'use branch.<name>.remote if possible' '
	...
	git config branch.default.remote other &&
...
test_expect_success 'confuses pattern as remote when no remote specified' '
	...
	# fetch <branch>.
	# We could just as easily have used "default"; the "*" emphasizes its
	# role as a pattern.
...
test_expect_success 'ls-remote --symref' '
	git fetch origin &&
	echo "ref: refs/heads/default	HEAD" >expect &&
	generate_references \
		HEAD \
		refs/heads/default >>expect &&
...
test_expect_success 'ls-remote patterns work with all protocol versions' '
	git for-each-ref --format="%(objectname)	%(refname)" \
		refs/heads/default refs/remotes/origin/default >expect &&
	git -c protocol.version=1 ls-remote . default >actual.v1 &&
	test_cmp expect actual.v1 &&
	git -c protocol.version=2 ls-remote . default >actual.v2 &&
