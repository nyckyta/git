...
test_expect_success 'setup conflicting branches' '
	...
	git checkout -b conflicting-a default &&
	echo a >conflicting &&
	git add conflicting &&
	git commit -m conflicting-a &&
	git checkout -b conflicting-b default &&
	echo b >conflicting &&
	git add conflicting &&
...
test_expect_success 'with no hook (merge)' '
	...
	git checkout side &&
	git merge -m "merge default" default &&
	git checkout default &&
	test_path_is_missing actual_hooks
'
...
test_expect_success '--no-verify with no hook (merge)' '
	...
	git branch -f side side-orig &&
	git checkout side &&
	git merge --no-verify -m "merge default" default &&
	git checkout default &&
	test_path_is_missing actual_hooks
'
...
test_expect_success 'with failing hook (merge)' '
	...
	echo "$PREMERGE" >expected_hooks &&
	git checkout side &&
	test_must_fail git merge -m "merge default" default &&
	git checkout default &&
	test_cmp expected_hooks actual_hooks
'
