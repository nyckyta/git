...
test_expect_success 'fetching multiple refs' '
	...
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	GIT_TRACE_PACKET="$(pwd)/log" git -C local fetch origin default baz &&

	git -C "$REPO" rev-parse "default" "baz" >expected &&
	git -C local rev-parse refs/remotes/origin/default refs/remotes/origin/baz >actual &&
	test_cmp expected actual &&
	grep "want-ref refs/heads/default" log &&
...
test_expect_success 'fetching ref and exact OID' '
	...
	GIT_TRACE_PACKET="$(pwd)/log" git -C local fetch origin \
		default "$oid":refs/heads/actual &&

	git -C "$REPO" rev-parse "default" "b" >expected &&
	git -C local rev-parse refs/remotes/origin/default refs/heads/actual >actual &&
	test_cmp expected actual &&
	grep "want $oid" log &&
	grep "want-ref refs/heads/default" log
'
...
test_expect_success 'server is initially ahead - no ref in want' '
	...
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	inconsistency default $(test_oid numeric) &&
...
test_expect_success 'server is initially ahead - ref in want' '
	...
	git -C "$REPO" rev-parse --verify default >expected &&
	git -C local rev-parse --verify refs/remotes/origin/default >actual &&
	test_cmp expected actual
'
...
test_expect_success 'server loses a ref - ref in want' '
	...
	rm -rf local &&
	cp -r "$LOCAL_PRISTINE" local &&
	echo "s/default/defined/" >"$HTTPD_ROOT_PATH/one-time-perl" &&
	test_must_fail git -C local fetch 2>err &&

	test_i18ngrep "fatal: remote error: unknown ref refs/heads/defined" err
'
