...
test_expect_success setup '
	...
	git branch side &&
	echo "11 $T" >>original &&
	git commit -a -m"default updates a bit." &&

	echo "12 $T" >>original &&
	git commit -a -m"default updates a bit more." &&
...
test_expect_success 'reference merge' '
	git merge -s recursive -m "reference merge" default
'

PRE_REBASE=$(git rev-parse test-rebase)
test_expect_success rebase '
	git checkout test-rebase &&
	GIT_TRACE=1 git rebase --merge default
'
...
test_expect_success 'rebase -Xtheirs' '
	git checkout -b conflicting default~2 &&
	echo "AB $T" >> original &&
	git commit -mconflicting original &&
	git rebase -Xtheirs default &&
	grep AB original &&
	! grep 11 original
'

test_expect_success 'rebase -Xtheirs from orphan' '
	git checkout --orphan orphan-conflicting default~2 &&
	echo "AB $T" >> original &&
...
test_expect_success 'picking rebase' '
	git reset --hard side &&
	git rebase --merge --onto default side^^ &&
	mb=$(git merge-base default HEAD) &&
	if test "$mb" = "$(git rev-parse default)"
	then
		echo happy
	else
		git show-branch
...
test_expect_success 'rebase -s funny -Xopt' '
	...
	git reset --hard &&
	git checkout -b test-funny default^ &&
	test_commit funny &&
	(
		PATH=./test-bin:$PATH &&
		git rebase -s funny -Xopt default
	) &&
...
test_expect_success '--reapply-cherry-picks' '
	...
	test_must_fail git -C repo rebase --merge default 2> err &&
	test_i18ngrep "error: could not apply.*add 12 in another branch" err &&
	git -C repo rebase --abort &&

	# With --reapply-cherry-picks, it works
	git -C repo rebase --merge --reapply-cherry-picks default
'

test_expect_success '--reapply-cherry-picks refrains from reading unneeded blobs' '
	git init server &&

	# O(1-10) -- O(1-11) -- O(1-12) default
	#        \
	#         -- O(0-10) otherbranch
	...
	git -C client checkout origin/default &&
	git -C client checkout origin/otherbranch &&

	# Sanity check to ensure that the blobs from the merge base and "add
	# 11" are missing
	git -C client rev-list --objects --all --missing=print >missing_list &&
	MERGE_BASE_BLOB=$(git -C server rev-parse default^^:file.txt) &&
	ADD_11_BLOB=$(git -C server rev-parse default^:file.txt) &&
	grep "[?]$MERGE_BASE_BLOB" missing_list &&
	grep "[?]$ADD_11_BLOB" missing_list &&

	git -C client rebase --merge --reapply-cherry-picks origin/default &&
