...
test_expect_success 'merge from unborn branch' '
	...
	test_when_finished "git checkout -f default" &&
	git rm -fr . &&
	test_tick &&
	git merge --ff-only c1 &&
	verify_merge file result.1 &&
...
test_expect_success 'merge c1 with c2 (no-commit in config)' '
	git reset --hard c1 &&
	test_config branch.default.mergeoptions "--no-commit" &&
	git merge c2 &&
	verify_merge file result.1-5 &&
	verify_head $c1 &&
	verify_mergeheads $c2
...
test_expect_success 'merge c1 with c2 (log in config)' '
	...
	test_config branch.default.mergeoptions "--log" &&
	git reset --hard c1 &&
	git merge c2 &&
...
test_expect_success 'merge c1 with c2 (log in config gets overridden)' '
	...
	test_config branch.default.mergeoptions "--no-log" &&
...
test_expect_success 'merge c1 with c2 (squash in config)' '
	git reset --hard c1 &&
	test_config branch.default.mergeoptions "--squash" &&
	git merge c2 &&
	verify_merge file result.1-5 &&
	verify_head $c1 &&
	verify_no_mergehead &&
...
test_expect_success 'override config option -n with --summary' '
	git reset --hard c1 &&
	test_config branch.default.mergeoptions "-n" &&
	test_tick &&
...
test_expect_success 'override config option --stat' '
	git reset --hard c1 &&
	test_config branch.default.mergeoptions "--stat" &&
	test_tick &&
...
test_expect_success 'combine branch.default.mergeoptions with merge.ff' '
	git reset --hard c0 &&
	test_config branch.default.mergeoptions "--ff" &&
	test_config merge.ff "false" &&
	test_tick &&
	git merge c1 &&
	verify_merge file result.1 &&
...
test_expect_success 'merge c0 with c1 (ff overrides no-ff)' '
	git reset --hard c0 &&
	test_config branch.default.mergeoptions "--no-ff" &&
	git merge --ff c1 &&
	verify_merge file result.1 &&
	verify_head $c1
'
...
test_expect_success 'set up mod-256 conflict scenario' '
	...
	# one side changes the first line of each to "default"
	sed s/-1/-default/ file >tmp &&
	mv tmp file &&
...
test_expect_success 'merge detects mod-256 conflicts (recursive)' '
	git reset --hard &&
	test_must_fail git merge -s recursive default
'

test_expect_success 'merge detects mod-256 conflicts (resolve)' '
	git reset --hard &&
	test_must_fail git merge -s resolve default
'
