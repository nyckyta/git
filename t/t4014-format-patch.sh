...
test_expect_success setup '
	...
	test_tick &&
	git commit -m "default accepts moral equivalent of #2" &&
...
test_expect_success 'format-patch --ignore-if-in-upstream' '
	git format-patch --stdout default..side >patch0 &&
...
test_expect_success 'format-patch --ignore-if-in-upstream' '
	git format-patch --stdout \
		--ignore-if-in-upstream default..side >patch1 &&
...
test_expect_success 'format-patch result applies' '
	git checkout -b rebuild-0 default &&
	git am -3 patch0 &&
	git rev-list default.. >list &&
	test_line_count = 2 list
'

test_expect_success 'format-patch --ignore-if-in-upstream result applies' '
	git checkout -b rebuild-1 default &&
	git am -3 patch1 &&
	git rev-list default.. >list &&
	test_line_count = 2 list
'
...
" &&
	git format-patch --stdout default..side >patch2 &&
...
test_expect_success 'extra headers without newlines' '
	...
	git config --add format.headers "Cc: S E Cipient <scipient@example.com>" &&
	git format-patch --stdout default..side >patch3 &&
...
test_expect_success 'extra headers with multiple To:s' '
	...
	git format-patch --stdout default..side >patch4 &&
...
test_expect_success 'additional command line cc (ascii)' '
	...
	git format-patch --cc="S E Cipient <scipient@example.com>" --stdout default..side >patch5 &&
...
test_expect_success 'command line headers' '
	...
	git format-patch --add-header="Cc: R E Cipient <rcipient@example.com>" --stdout default..side >patch6 &&
...
test_expect_success 'configuration headers and command line headers' '
	git config --replace-all format.headers "Cc: R E Cipient <rcipient@example.com>" &&
	git format-patch --add-header="Cc: S E Cipient <scipient@example.com>" --stdout default..side >patch7 &&
...
test_expect_success 'command line To: header (ascii)' '
	git config --unset-all format.headers &&
	git format-patch --to="R E Cipient <rcipient@example.com>" --stdout default..side >patch8 &&
...
test_expect_failure 'command line To: header (rfc2047)' '
	git format-patch --to="R Ä Cipient <rcipient@example.com>" --stdout default..side >patch8 &&
	sed -e "/^\$/q" patch8 >hdrs8 &&
...
test_expect_success 'configuration To: header (ascii)' '
	...
	git format-patch --stdout default..side >patch9 &&
...
test_expect_success 'format.from=false' '
	git -c format.from=false format-patch --stdout default..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch patch &&
...
test_expect_success 'format.from=true' '
	git -c format.from=true format-patch --stdout default..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
	grep "^From: C O Mitter <committer@example.com>\$" hdrs
'
...
test_expect_success 'format.from with address' '
	git -c format.from="F R Om <from@example.com>" format-patch --stdout default..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
...
test_expect_success '--no-from overrides format.from' '
	git -c format.from="F R Om <from@example.com>" format-patch --no-from --stdout default..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
	! grep "^From: F R Om <from@example.com>\$" hdrs
'
...
test_expect_success '--from overrides format.from' '
	git -c format.from="F R Om <from@example.com>" format-patch --from --stdout default..side >patch &&
	sed -e "/^\$/q" patch >hdrs &&
	check_patch hdrs &&
	! grep "^From: F R Om <from@example.com>\$" hdrs
'
...
test_expect_success '--no-to overrides config.to' '
	...
	git format-patch --no-to --stdout default..side >patch10 &&
...
test_expect_success '--no-cc overrides config.cc' '
	...
	git format-patch --no-cc --stdout default..side >patch12 &&
...
test_expect_success '--no-add-header overrides config.headers' '
	...
	git format-patch --no-add-header --stdout default..side >patch13 &&
...
test_expect_success 'multiple files' '
	rm -rf patches/ &&
	git checkout side &&
	git format-patch -o patches/ default &&
	ls patches/0001-Side-changes-1.patch patches/0002-Side-changes-2.patch patches/0003-Side-changes-3-with-n-backslash-n-in-it.patch
'

test_expect_success 'reroll count' '
	rm -fr patches &&
	git format-patch -o patches --cover-letter --reroll-count 4 default..side >list &&
...
test_expect_success 'reroll count (-v)' '
	rm -fr patches &&
	git format-patch -o patches --cover-letter -v4 default..side >list &&
	! grep -v "^patches/v4-000[0-3]-" list &&
	sed -n -e "/^Subject: /p" $(cat list) >subjects &&
	! grep -v "^Subject: \[PATCH v4 [0-3]/3\] " subjects
'
...
test_expect_success 'no threading' '
	git checkout side &&
	check_threading expect.no-threading default
'
...
test_expect_success 'thread' '
	check_threading expect.thread --thread default
'
...
test_expect_success 'thread cover-letter' '
	check_threading expect.cover-letter --cover-letter --thread default
'
...
test_expect_success 'thread cover-letter in-reply-to' '
	check_threading expect.cl-irt --cover-letter \
		--in-reply-to="<test.message>" --thread default
'

test_expect_success 'thread explicit shallow' '
	check_threading expect.cl-irt --cover-letter \
		--in-reply-to="<test.message>" --thread=shallow default
'
...
test_expect_success 'thread deep' '
	check_threading expect.deep --thread=deep default
'
...
test_expect_success 'thread deep in-reply-to' '
	check_threading expect.deep-irt  --thread=deep \
		--in-reply-to="<test.message>" default
'
...
test_expect_success 'thread deep cover-letter' '
	check_threading expect.deep-cl --cover-letter --thread=deep default
'
...
test_expect_success 'thread deep cover-letter in-reply-to' '
	check_threading expect.deep-cl-irt --cover-letter \
		--in-reply-to="<test.message>" --thread=deep default
'

test_expect_success 'thread via config' '
	test_config format.thread true &&
	check_threading expect.thread default
'

test_expect_success 'thread deep via config' '
	test_config format.thread deep &&
	check_threading expect.deep default
'
...
test_expect_success 'thread config + --no-thread' '
	test_config format.thread deep &&
	check_threading expect.no-threading --no-thread default
'
...
test_expect_success 'excessive subject' '
	...
	git format-patch -o patches/ default..side &&
...
test_expect_success 'format-patch --numstat should produce a patch' '
	git format-patch --numstat --stdout default..side >output &&
	grep "^diff --git a/" output >diff &&
	test_line_count = 5 diff
'

test_expect_success 'format-patch -- <path>' '
	git format-patch default..side -- file 2>error &&
	! grep "Use .--" error
'
...
body" &&
	test_must_fail git format-patch --cover-letter --cover-from-description garbage default &&
	test_config format.coverFromDescription garbage &&
	test_must_fail git format-patch --cover-letter default
'
...
body" &&
	...
	git format-patch --stdout --cover-letter default >actual &&
...
body" &&
	git checkout rebuild-1 &&
	git format-patch --stdout --cover-letter --cover-from-description default default >actual &&
	grep "^Subject: \[PATCH 0/2\] \*\*\* SUBJECT HERE \*\*\*$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	grep "^config subject$" actual &&
	grep "^body$" actual
...
body" &&
	git checkout rebuild-1 &&
	git format-patch --stdout --cover-letter --cover-from-description none default >actual &&
	grep "^Subject: \[PATCH 0/2\] \*\*\* SUBJECT HERE \*\*\*$" actual &&
	grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	! grep "^config subject$" actual &&
	! grep "^body$" actual
...
body" &&
	git checkout rebuild-1 &&
	git format-patch --stdout --cover-letter --cover-from-description message default >actual &&
	grep "^Subject: \[PATCH 0/2\] \*\*\* SUBJECT HERE \*\*\*$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	grep "^config subject$" actual &&
	grep "^body$" actual
...
body" &&
	git checkout rebuild-1 &&
	git format-patch --stdout --cover-letter --cover-from-description subject default >actual &&
	grep "^Subject: \[PATCH 0/2\] config subject$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	! grep "^config subject$" actual &&
	grep "^body$" actual
...
body" &&
	git checkout rebuild-1 &&
	git format-patch --stdout --cover-letter --cover-from-description auto default >actual &&
	grep "^Subject: \[PATCH 0/2\] config subject$" actual &&
	! grep "^\*\*\* BLURB HERE \*\*\*$" actual &&
	! grep "^config subject$" actual &&
	grep "^body$" actual
...
test_expect_success 'cover letter using branch description (3)' '
	git checkout rebuild-1 &&
	test_config branch.rebuild-1.description hello &&
	git format-patch --stdout --cover-letter ^default rebuild-1 >actual &&
	grep hello actual
'
...
test_expect_success 'format-patch -o with no leading directories' '
	rm -fr patches &&
	git format-patch -o patches default..side &&
	count=$(git rev-list --count default..side) &&
	ls patches >list &&
...
test_expect_success 'format-patch -o with leading existing directories' '
	rm -rf existing-dir &&
	mkdir existing-dir &&
	git format-patch -o existing-dir/patches default..side &&
	count=$(git rev-list --count default..side) &&
...
test_expect_success 'format-patch -o with leading non-existing directories' '
	...
	git format-patch -o non-existing-dir/patches default..side &&
	count=$(git rev-list --count default..side) &&
...
test_expect_success 'format-patch -o overrides format.outputDirectory' '
	...
	git format-patch default..side -o patchset &&
...
test_expect_success 'format-patch --base=auto' '
	git checkout -b upstream default &&
...
test_expect_success 'format-patch errors out when history involves criss-cross' '
	...
	git checkout default &&
	test_commit A &&
	git checkout -b xb default &&
	test_commit B &&
	git checkout -b xc default &&
	test_commit C &&
