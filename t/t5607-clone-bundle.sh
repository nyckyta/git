...
test_expect_success '"verify" needs a worktree' '
	git bundle create tip.bundle -1 default &&
...
test_expect_failure 'bundle --stdin' '
	echo default | git bundle create stdin-bundle.bdl --stdin &&
	git ls-remote stdin-bundle.bdl >output &&
	grep default output
'

test_expect_failure 'bundle --stdin <rev-list options>' '
	echo default | git bundle create hybrid-bundle.bdl --stdin tag &&
...
test_expect_success 'failed bundle creation does not leave cruft' '
	...
	test_must_fail git bundle create fail.bundle default..default &&
...
test_expect_success 'fetch SHA-1 from bundle' '
	...
	test_commit -C foo x &&
	git -C foo bundle create tip.bundle -1 default &&
