...
test_expect_success setup '
	...
	git reflog refs/heads/default >output &&
...
test_expect_success 'delete' '
	...
	master_entry_count=$(git reflog show default | wc -l) &&

	test $HEAD_entry_count = 5 &&
	test $master_entry_count = 5 &&


	git reflog delete default@{1} &&
	git reflog show default > output &&
	...
	test $master_entry_count = $(git reflog show default | wc -l) &&

	HEAD_entry_count=$(git reflog | wc -l) &&

	git reflog delete default@{07.04.2005.15:15:00.-0700} &&
	git reflog show default > output &&
	test_line_count = $(($master_entry_count - 1)) output &&
	! grep dragon < output
...
test_expect_success 'git reflog expire unknown reference' '
	...
	test_must_fail git reflog expire default@{123} 2>stderr &&
...
test_expect_success 'checkout should not delete log for packed ref' '
	test $(git reflog default | wc -l) = 4 &&
	git branch foo &&
	git pack-refs --all &&
	git checkout foo &&
	test $(git reflog default | wc -l) = 4
...
test_expect_success 'stale dirs do not cause d/f conflicts (reflogs on)' '
	...
	git branch one/two default &&
	echo "one/two@{0} branch: Created from default" >expect &&
	...
	git branch one default &&
	echo "one@{0} branch: Created from default" >expect &&
...
test_expect_success 'stale dirs do not cause d/f conflicts (reflogs off)' '
	...
	git -c core.logallrefupdates=false branch one default &&
	git log -g --format="%gd %gs" one >actual &&
