...
test_expect_success 'A: verify commit' '
	...
	initial
	EOF
	git cat-file commit default | sed 1d >actual &&
...
test_expect_success 'A: verify tree' '
	...
	100755 blob file4
	EOF
	git cat-file -p default^{tree} | sed "s/ [0-9a-f]*	/ /" >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify file2' '
	echo "$file2_data" >expect &&
	git cat-file blob default:file2 >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify file3' '
	echo "$file3_data" >expect &&
	git cat-file blob default:file3 >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify file4' '
	printf "$file4_data" >expect &&
	git cat-file blob default:file4 >actual &&
	test_cmp expect actual
'

test_expect_success 'A: verify tag/series-A' '
	cat >expect <<-EOF &&
	object $(git rev-parse refs/heads/default)
	type commit
	tag series-A
...
test_expect_success 'A: verify tag/series-A-blob' '
	cat >expect <<-EOF &&
	object $(git rev-parse refs/heads/default:file3)
	type blob
	tag series-A-blob
...
test_expect_success 'A: verify marks output' '
	cat >expect <<-EOF &&
	:2 $(git rev-parse --verify default:file2)
	:3 $(git rev-parse --verify default:file3)
	:4 $(git rev-parse --verify default:file4)
	:5 $(git rev-parse --verify default^0)
	:6 $(git cat-file tag nested | grep object | cut -d" " -f 2)
	:7 $(git rev-parse --verify nested)
	:8 $(git rev-parse --verify default^0)
	EOF
...
test_expect_success 'A: tag blob by sha1' '
	...
	cat >input <<-INPUT_END &&
	tag series-A-blob-2
	from $(git rev-parse refs/heads/default:file3)
	data <<EOF
	Tag blob by sha1.
	EOF
...
test_expect_success 'A: verify diff' '
	...
	EOF
	git diff-tree -M -r default verify--import-marks >actual &&
	compare_diff_raw expect actual &&
	test $(git rev-parse --verify default:file2) \
...
test_expect_success 'B: accept branch name "TEMP_TAG"' '
	...
	git fast-import <input &&
	test -f .git/TEMP_TAG &&
	test $(git rev-parse default) = $(git rev-parse TEMP_TAG^)
'
...
test_expect_success 'C: incremental import create pack from stdin' '
	...
	oldf=$(git rev-parse --verify default:file2) &&
	test_tick &&
	cat >input <<-INPUT_END &&
	commit refs/heads/branch
	committer $GIT_COMMITTER_NAME <$GIT_COMMITTER_EMAIL> $GIT_COMMITTER_DATE
...
test_expect_success 'C: verify commit' '
	cat >expect <<-EOF &&
	parent $(git rev-parse --verify default^0)
	author $GIT_COMMITTER_NAME <$GIT_COMMITTER_EMAIL> $GIT_COMMITTER_DATE
	committer $GIT_COMMITTER_NAME <$GIT_COMMITTER_EMAIL> $GIT_COMMITTER_DATE
...
test_expect_success 'C: validate rename result' '
	...
	EOF
	git diff-tree -M -r default branch >actual &&
	compare_diff_raw expect actual
'
...
test_expect_success 'P: superproject & submodule mix' '
	...
	(
		cd sub &&
		git init &&
		git fetch --update-head-ok .. refs/heads/sub:refs/heads/default &&
...
test_expect_success 'Y: rewrite submodules' '
	...
	(
		...
		test "$(git rev-parse default)" = "$(test_oid Ymaster)"
	)
'
