...
test_expect_success 'setup' '
	# Make two branches, "default" and "side"
	echo one >file &&
	git add file &&
	git commit -m one &&
	echo two >file &&
	...
	# default --single that follows HEAD=default
	git clone --single-branch . dir_master &&

	# default --single that follows HEAD=default with no tags
	...
	# advance both "default" and "side" branches
	git checkout side &&
	echo five >file &&
	git commit -a -m five &&
	git checkout default &&
...
test_expect_success '--single-branch while HEAD pointing at default' '
	(
		cd dir_master &&
		git fetch --force &&
		git for-each-ref refs/remotes/origin >refs &&
...
test_expect_success '--single-branch while HEAD pointing at default and --no-tags' '
	(
