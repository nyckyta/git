...
test_expect_success 'non-interactive rebase --continue works with touched file' '
	...
	test_must_fail git rebase --onto default default topic &&
	echo "Resolved" >F2 &&
	git add F2 &&
	test-tool chmtime =-60 F1 &&
	git rebase --continue
...
test_expect_success 'rebase --continue remembers merge strategy and options' '
	...
	(
		PATH=./test-bin:$PATH &&
		test_must_fail git rebase -s funny -Xopt default topic
	) &&
...
test_expect_success 'rebase -i --continue handles merge strategy and options' '
	...
	(
		PATH=./test-bin:$PATH &&
		test_must_fail git rebase -i -s funny -Xopt -Xfoo default topic
	) &&
	test -f funny.was.run &&
	rm funny.was.run &&
	echo "Resolved" >F2 &&
...
test_expect_success 'setup rerere database' '
	...
	test_must_fail git rebase -m default topic &&
	echo "Resolved" >F2 &&
	cp F2 expected-F2 &&
	git add F2 &&
...
test_rerere_autoupdate () {
	...
	test_expect_success "rebase $action --continue remembers --rerere-autoupdate" '
		prepare &&
		test_must_fail git rebase $action --rerere-autoupdate default topic &&
	...
	test_expect_success "rebase $action --continue honors rerere.autoUpdate" '
		...
		test_must_fail git rebase $action default topic &&
		test_cmp expected-F2 F2 &&
		git diff-files --quiet &&
		test_must_fail git rebase --continue &&
		test_cmp expected-F3 F3 &&
	...
	test_expect_success "rebase $action --continue remembers --no-rerere-autoupdate" '
		prepare &&
		test_config rerere.autoupdate true &&
		test_must_fail git rebase $action --no-rerere-autoupdate default topic &&
		test_cmp expected-F2 F2 &&
