...
test_expect_success 'pushing into a repository using a ref namespace' '
	(
		cd original &&
		git push pushee-namespaced default &&
		git ls-remote pushee-namespaced >actual &&
		printf "$commit1\trefs/heads/default\n" >expected &&
...
test_expect_success 'pulling from a repository using a ref namespace' '
	(
		...
		printf "$commit1 commit\trefs/remotes/pushee-namespaced/default\n" >expected &&
...
test_expect_success 'mirroring a repository using a ref namespace' '
	...
	(
		cd mirror &&
		git for-each-ref refs/ >actual &&
		printf "$commit1 commit\trefs/namespaces/namespace/refs/heads/default\n" >expected &&
...
test_expect_success 'try to update a hidden ref' '
	test_config -C pushee transfer.hideRefs refs/heads/default &&
	test_must_fail git -C original push pushee-namespaced default
'

test_expect_success 'try to update a ref that is not hidden' '
	test_config -C pushee transfer.hideRefs refs/namespaces/namespace/refs/heads/default &&
	git -C original push pushee-namespaced default
'
...
test_expect_success 'denyCurrentBranch and unborn branch with ref namespace' '
	(
		...
		test_must_fail git push unborn-namespaced HEAD:default &&
		git -C unborn config receive.denyCurrentBranch updateInstead &&
		git push unborn-namespaced HEAD:default
	)
'
