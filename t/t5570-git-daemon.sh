...
test_expect_success 'push disabled'      "test_remote_error    '$msg' push  repo.git default"
...
test_expect_success 'push disabled'      "test_remote_error    'service not enabled'     push  repo.git default"
...
test_expect_success FAKENC 'hostname interpolation works after LF-stripping' '
	...
	# just pick out the value of default, which avoids any protocol
	# particulars
	perl -lne "print \$1 if m{^(\\S+) refs/heads/default}" <output.raw >actual &&
	git -C "$repo" rev-parse default >expect &&
