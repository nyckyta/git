...
test_expect_success 'push if submodule has no remote' '
	...
	(
		...
		git push --recurse-submodules=check ../pub.git default
	)
'
...
test_expect_success 'push fails if submodule commit not on remote' '
	...
	(
		...
		# on the command line...
		test_must_fail git push --recurse-submodules=check ../pub.git default &&

		# ...or if specified in the configuration..
		test_must_fail git -c push.recurseSubmodules=check push ../pub.git default
	)
'
...
test_expect_success 'push succeeds if submodule commit not on remote but using on-demand on command line' '
	...
	(
		...
		git push --recurse-submodules=on-demand ../pub.git default &&
		# Check that the supermodule commit got there
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
		# Check that the submodule commit got there too
		cd gar/bage &&
		git diff --quiet origin/default default
	)
'
...
test_expect_success 'push succeeds if submodule commit not on remote but using on-demand from config' '
	...
	(
		...
		git -c push.recurseSubmodules=on-demand push ../pub.git default &&
		# Check that the supermodule commit got there
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
		# Check that the submodule commit got there too
...
test_expect_success 'push succeeds if submodule commit not on remote but using auto-on-demand via submodule.recurse config' '
	...
	(
		...
		git -c submodule.recurse push ../pub.git default &&
		# Check that the supermodule commit got there
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
		# Check that the submodule commit got there too
...
test_expect_success 'push recurse-submodules on command line overrides config' '
	...
	(
		...
		test_must_fail git -c push.recurseSubmodules=on-demand push --recurse-submodules=check ../pub.git default &&
		# Check that the supermodule commit did not get there
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default^ &&
		# Check that the submodule commit did not get there
		(cd gar/bage && git diff --quiet origin/default default^) &&

		# Ensure that we can override check in the config to
		# disable submodule recursion entirely
		(cd gar/bage && git diff --quiet origin/default default^) &&
		git -c push.recurseSubmodules=on-demand push --recurse-submodules=no ../pub.git default &&
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
		(cd gar/bage && git diff --quiet origin/default default^) &&
		...
		git -c push.recurseSubmodules=on-demand push --no-recurse-submodules ../pub.git default &&
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
		(cd gar/bage && git diff --quiet origin/default default^) &&
		...
		git -c push.recurseSubmodules=check push --recurse-submodules=on-demand ../pub.git default &&
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
		(cd gar/bage && git diff --quiet origin/default default)
	)
...
test_expect_success 'push recurse-submodules last one wins on command line' '
	...
	(
		...
		test_must_fail git push --recurse-submodules=on-demand --recurse-submodules=check ../pub.git default &&
		# Check that the supermodule commit did not get there
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default^ &&
		# Check that the submodule commit did not get there
		...
		# should result in "no"
		git push --recurse-submodules=on-demand --recurse-submodules=no ../pub.git default &&
		...
		# should result in "no"
		git push --recurse-submodules=on-demand --no-recurse-submodules ../pub.git default &&
		# Check that the submodule commit did not get there
		(cd gar/bage && git diff --quiet origin/default default^) &&

		# But the options in the other order should push the submodule
		git push --recurse-submodules=check --recurse-submodules=on-demand ../pub.git default &&
...
test_expect_success 'push succeeds if submodule commit disabling recursion from cmdline overriding config' '
	...
	(
		...
		git -c push.recurseSubmodules=check push --recurse-submodules=no ../pub.git default &&
		# Check that the supermodule commit got there
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
...
test_expect_success 'push succeeds if submodule commit disabling recursion from cmdline (alternative form) overriding config' '
	...
	(
		...
		git -c push.recurseSubmodules=check push --no-recurse-submodules ../pub.git default &&
		# Check that the supermodule commit got there
		git fetch ../pub.git &&
		git diff --quiet FETCH_HEAD default &&
		# But that the submodule commit did not
...
test_expect_success 'submodule entry pointing at a tag is error' '
	...
	test_must_fail git -C work push --recurse-submodules=on-demand ../pub.git default 2>err &&
...
test_expect_success 'push fails if recurse submodules option passed as yes' '
	...
	(
		...
		test_must_fail git push --recurse-submodules=yes ../pub.git default &&
		test_must_fail git -c push.recurseSubmodules=yes push ../pub.git default &&
		git push --recurse-submodules=on-demand ../pub.git default
	)
'
...
test_expect_success 'push succeeds if submodule has no remote and is on the first superproject commit' '
	...
	(
		...
		git push --recurse-submodules=check origin default
	)
'
...
test_expect_success 'push unpushed submodules when not needed' '
	(
		...
		(
			...
			git push &&
			git rev-parse origin/default >../../../expected
		) &&
		git checkout default &&
		git add gar/bage &&
...
test_expect_success 'push unpushable submodule recursively fails' '
	(
		...
		test_must_fail git push --recurse-submodules=on-demand ../pub.git default
	) &&
	(
		cd submodule.git &&
		git rev-parse default >../actual
	) &&
	test_when_finished git -C work reset --hard default^ &&
	test_cmp expected actual
'
...
test_expect_success 'push --dry-run does not recursively update submodules' '
	(
		cd work/gar/bage &&
		git checkout default &&
		git rev-parse default >../../../expected_submodule &&
		> junk9 &&
		git add junk9 &&
		...
		cd ../.. &&
		git checkout default &&
		git rev-parse default >../expected_pub &&
		git add gar/bage &&
		git commit -m "Ninth commit for gar/bage" &&
		git push --dry-run --recurse-submodules=on-demand ../pub.git default
	) &&
	git -C submodule.git rev-parse default >actual_submodule &&
	git -C pub.git rev-parse default >actual_pub &&
	test_cmp expected_pub actual_pub &&
	test_cmp expected_submodule actual_submodule
'

test_expect_success 'push --dry-run does not recursively update submodules' '
	git -C work push --dry-run --recurse-submodules=only ../pub.git default &&
...
test_expect_success 'push only unpushed submodules recursively' '
	git -C work/gar/bage rev-parse default >expected_submodule &&
	git -C pub.git rev-parse default >expected_pub &&

	git -C work push --recurse-submodules=only ../pub.git default &&
...
test_expect_success 'push propagating the remotes name to a submodule' '
	...
	test_must_fail git -C work push --recurse-submodules=on-demand pub default &&
	# Succeeds when submodules has matching remote and refspec
	git -C work push --recurse-submodules=on-demand origin default &&

	git -C submodule.git rev-parse default >actual_submodule &&
	git -C pub.git rev-parse default >actual_pub &&
	git -C work/gar/bage rev-parse default >expected_submodule &&
	git -C work rev-parse default >expected_pub &&
	test_cmp expected_submodule actual_submodule &&
	test_cmp expected_pub actual_pub
'
...
test_expect_success 'push propagating refspec to a submodule' '
	...
	git -C work/gar/bage branch branch2 default &&
