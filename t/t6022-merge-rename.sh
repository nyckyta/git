...
test_expect_success 'setup' '
	...
	sed -e "/^g /s/.*/g : default changes a line/" <A >A+ &&
	mv A+ A &&
	git commit -a -m "default updates A" &&
...
'
	git reset --hard &&
	git show-branch &&
	test_expect_code 1 git pull . default &&
	git ls-files -u B >b.stages &&
	test_line_count = 3 b.stages &&
	git ls-files -s N >n.stages &&
	test_line_count = 1 n.stages &&
...
test_expect_success 'merge default into rename has correct extended markers' '
	git checkout rename^0 &&
	test_must_fail git merge -s recursive default^0 &&
	...
	=======
	8.5
	>>>>>>> default^0:original_file
	EOF
	test_cmp expected renamed_file
'

test_expect_success 'merge rename into default has correct extended markers' '
	git reset --hard &&
