...
sub get_last_local_revision {
	...
	my $note = run_git("notes --ref=${remotename}/mediawiki show refs/mediawiki/${remotename}/default 2>/dev/null");
...
sub mw_list {
	...
	# called default, and HEAD pointing to it.
	print {*STDOUT} "? refs/heads/default\n";
	print {*STDOUT} "\@refs/heads/default HEAD\n";
	print {*STDOUT} "\n";
	return;
}
...
sub import_file_revision {
	...
	print {*STDOUT} "commit refs/mediawiki/${remotename}/default\n";
	print {*STDOUT} "mark :${n}\n";
	...
	if (!$full_import && $n == 1) {
		print {*STDOUT} "from refs/mediawiki/${remotename}/default^0\n";
	}
...
sub mw_import_ref {
	...
	# "import refs/heads/default".
	# Since HEAD is a symbolic ref to default (by convention,
...
sub mw_push {
	...
	for my $refspec (@refsspecs) {
		...
		if ($remote ne 'refs/heads/default') {
			print {*STDERR} "Only push to the branch 'default' is supported on a MediaWiki\n";
			print {*STDOUT} "error ${remote} only default allowed\n";
			next;
		}
...
sub mw_push_revision {
	my $local = shift;
	my $remote = shift; # actually, this has to be "refs/heads/default" at this point.
	...
	chomp($HEAD_sha1);
	# Get sha1 of commit pointed by remotes/$remotename/default
	my $remoteorigin_sha1 = run_git("rev-parse refs/remotes/${remotename}/default 2>/dev/null");
	...
	# Get every commit in between HEAD and refs/remotes/origin/default,
	# including HEAD and refs/remotes/origin/default
	my @commit_pairs = ();
	if ($last_local_revid > 0) {
