...
called 'foo' that was created on top of 'default'. You can run:

```
git format-patch -o output_dir default..foo
```
...
```
git fetch gitgitgadget git-gui/default
```

Then create a new branch based on git-gui/default:

```
git checkout -b <your-branch-name> git-gui/default
```
