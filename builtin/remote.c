...
static const char * const builtin_remote_usage[] = {
	...
	N_("git remote add [-t <branch>] [-m <default>] [-f] [--tags | --no-tags] [--mirror=<fetch|push>] <name> <url>"),
...
static int add(int argc, const char **argv)
{
	...
	struct option options[] = {
		...
		OPT_STRING('m', "default", &default_branch, N_("branch"), N_("default branch")),
	...
	if (mirror && default_branch)
		die(_("specifying a default branch makes no sense with --mirror"));
	...
	if (default_branch) {
		...
		strbuf_reset(&buf2);
		strbuf_addf(&buf2, "refs/remotes/%s/%s", name, default_branch);

		if (create_symref(buf.buf, buf2.buf, "remote add"))
			return error(_("Could not setup default '%s'"), default_branch);
	}
